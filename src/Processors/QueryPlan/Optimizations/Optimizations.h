...
/// Rewrite plan via the generic Reflection `ReadHint` dispatcher (vector ANN today).
size_t tryUseReflectionReadHint(QueryPlan::Node * parent_node, QueryPlan::Nodes & nodes, const Optimization::ExtraSettings &);
...
inline const auto & getOptimizations()
{
    static const std::array<Optimization, 19> optimizations = {{
        ...
        /// Reflection ReadHint must run before vector search: rewriting the plan into a Union
        /// causes the vector-search `typeid_cast` chain to no-op naturally.
        {tryUseReflectionReadHint, "useReflectionReadHint", &QueryPlanOptimizationSettings::try_use_ann_index},
