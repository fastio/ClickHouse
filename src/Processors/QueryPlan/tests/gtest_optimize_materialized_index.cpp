...
TEST(HintsExpansion, NonDistributed)
{
    ...
    hints.per_part[uuid_a] = makeNearestNeighbours({10, 20, 30}, {0.1f, 0.2f, 0.3f});
    hints.per_part[uuid_b] = makeNearestNeighbours({40, 50}, {0.4f, 0.5f});
    ...
    ASSERT_TRUE(rh_a.mi_search_results.has_value());
    EXPECT_EQ(rh_a.mi_search_results->rows, (std::vector<UInt64>{10, 20, 30}));
    ASSERT_TRUE(rh_a.mi_search_results->distances.has_value());
    EXPECT_EQ(rh_a.mi_search_results->distances->size(), 3u);

    ASSERT_TRUE(rh_b.mi_search_results.has_value());
    EXPECT_EQ(rh_b.mi_search_results->rows, (std::vector<UInt64>{40, 50}));

    EXPECT_FALSE(rh_c.mi_search_results.has_value());
...
TEST(HintsExpansion, DistributedBranch)
{
    ...
    MaterializedIndexHints hints;
    hints.per_part[uuid_a] = makeNearestNeighbours({100}, {0.01f});
    ...
    ASSERT_TRUE(rh_a.mi_search_results.has_value());
    EXPECT_EQ(rh_a.mi_search_results->rows, (std::vector<UInt64>{100}));
    EXPECT_FALSE(rh_b.mi_search_results.has_value());
}
...
TEST(HintsExpansion, DoubleWriteAssert)
{
    ...
    MaterializedIndexHints hints;
    hints.per_part[uuid_a] = makeNearestNeighbours({1}, {0.0f});
...
TEST(OptimizationOrder, MaterializedIndexFirst)
{
    ...
    auto mi_pos = find_index("useMaterializedIndex");
    auto vs_pos = find_index("useVectorSearch");

    ASSERT_TRUE(mi_pos.has_value());
    ASSERT_TRUE(vs_pos.has_value());
    EXPECT_LT(*mi_pos, *vs_pos);
}
