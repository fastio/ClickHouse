...
TEST(HintsExpansion, NonDistributed)
{
    ...
    attachAuxiliaryIndexHintForPart(uuid_a, rh_a, hints);
    attachAuxiliaryIndexHintForPart(uuid_b, rh_b, hints);
    attachAuxiliaryIndexHintForPart(uuid_c, rh_c, hints);

    ASSERT_TRUE(rh_a.auxiliary_index_search_results.has_value());
    EXPECT_EQ(rh_a.auxiliary_index_search_results->rows, (std::vector<UInt64>{10, 20, 30}));
    ASSERT_TRUE(rh_a.auxiliary_index_search_results->distances.has_value());
    EXPECT_EQ(rh_a.auxiliary_index_search_results->distances->size(), 3u);

    ASSERT_TRUE(rh_b.auxiliary_index_search_results.has_value());
    EXPECT_EQ(rh_b.auxiliary_index_search_results->rows, (std::vector<UInt64>{40, 50}));

    EXPECT_FALSE(rh_c.auxiliary_index_search_results.has_value());
...
TEST(HintsExpansion, DistributedBranch)
{
    ...
    ASSERT_TRUE(rh_a.auxiliary_index_search_results.has_value());
    EXPECT_EQ(rh_a.auxiliary_index_search_results->rows, (std::vector<UInt64>{100}));
    EXPECT_FALSE(rh_b.auxiliary_index_search_results.has_value());
}
...
TEST(HintsExpansion, DoubleWriteAssert)
{
    ...
    EXPECT_DEATH(attachAuxiliaryIndexHintForPart(uuid_a, rh_a, hints), "");
#endif
}
...
TEST(HintsExpansion, CoveredPartWithoutHitsAttachesEmptyResult)
{
    ...
    ASSERT_TRUE(rh_a.auxiliary_index_search_results.has_value());
    EXPECT_TRUE(rh_a.auxiliary_index_search_results->rows.empty());
    ASSERT_TRUE(rh_a.auxiliary_index_search_results->distances.has_value());
    EXPECT_TRUE(rh_a.auxiliary_index_search_results->distances->empty());

    EXPECT_FALSE(rh_b.auxiliary_index_search_results.has_value());
}

TEST(OptimizationOrder, AuxiliaryIndexFirst)
{
    ...
    auto auxiliary_index_pos = find_index("useAuxiliaryIndex");
    auto vs_pos = find_index("useVectorSearch");

    ASSERT_TRUE(auxiliary_index_pos.has_value());
    ASSERT_TRUE(vs_pos.has_value());
    EXPECT_LT(*auxiliary_index_pos, *vs_pos);
}
