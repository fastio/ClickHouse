...
{
    MergeTreeData::DataPartsVector old_ann_index_parts;
    std::vector<UUID> old_ann_index_part_uuids;
...
struct RebuildSourcePartCandidate
{
    ...
    std::vector<UUID> affected_ann_index_part_uuids;
...
/// ANNIndex's current snapshot for one cycle.
...
/// Stateless cycle-head helper. The cycle pulls the source / ann_index snapshots
...
{
public:
    /// Time complexity is O(|source| + |ann_index|).
