...
void CoverageMap::rebuildSourceMapNoLock()
{
    ...
    for (const auto & [_, entries] : ann_index_to_entries)
    {
...
{
    {
        ...
        ann_index_to_entries.clear();
        for (auto & [ann_index_uuid, ann_index_entries] : entries)
            ann_index_to_entries.emplace(ann_index_uuid, std::move(ann_index_entries));
        rebuildSourceMapNoLock();
    }
    cv.notify_all();
}

void CoverageMap::appendFromBuild(UUID ann_index_part_uuid, std::vector<CoverageEntry> entries)
{
    {
        std::unique_lock lock(mutex);
        ann_index_to_entries[ann_index_part_uuid] = std::move(entries);
        rebuildSourceMapNoLock();
    }
    cv.notify_all();
}
    ...
    std::vector<UUID> /*outgoing_source_uuids*/)
{
    {
        std::unique_lock lock(mutex);
        ann_index_to_entries.erase(retired_ann_index_part_uuid);
        ann_index_to_entries[new_ann_index_part_uuid] = std::move(incoming);
        rebuildSourceMapNoLock();
    }
    cv.notify_all();
}
...
{
    {
        ...
        {
            ann_index_to_entries.erase(commit.retired_ann_index_part_uuid);
            ann_index_to_entries[commit.new_ann_index_part_uuid] = std::move(commit.incoming);
        }
        rebuildSourceMapNoLock();
    }
    cv.notify_all();
...
{
    {
        ...
        for (const auto & retired_uuid : retired_ann_index_part_uuids)
            ann_index_to_entries.erase(retired_uuid);
        ann_index_to_entries[new_ann_index_part_uuid] = std::move(incoming);
        rebuildSourceMapNoLock();
    }
    cv.notify_all();
...
void CoverageMap::dropMiPart(UUID ann_index_part_uuid)
{
    {
        std::unique_lock lock(mutex);
        if (ann_index_to_entries.erase(ann_index_part_uuid) == 0)
            return;
        rebuildSourceMapNoLock();
    }
    cv.notify_all();
...
std::unordered_set<UUID> CoverageMap::miPartUuidsCoveringAnySourceUuid(const std::vector<UUID> & source_uuids) const
{
    ...
    for (const auto & [ann_index_part_uuid, entries] : ann_index_to_entries)
    {
        for (const auto & entry : entries)
        {
            if (wanted.contains(entry.source_part_uuid))
            {
                out.insert(ann_index_part_uuid);
                break;
            }
        }
    }
