...
std::unordered_map<UUID, CoverageEntry> buildCoverageBySourceUuid(
    const std::unordered_map<UUID, std::vector<CoverageEntry>> & coverage_by_ann_index_part_uuid)
{
    std::unordered_map<UUID, CoverageEntry> coverage_by_source_uuid;
    for (const auto & [_, entries] : coverage_by_ann_index_part_uuid)
    {
        for (const auto & entry : entries)
        {
...
std::vector<UUID> collectAffectedANNIndexPartUuids(
    ...
    const std::unordered_map<UUID, std::vector<CoverageEntry>> & coverage_by_ann_index_part_uuid)
{
    ...
    for (const auto & [ann_index_part_uuid, entries] : coverage_by_ann_index_part_uuid)
    {
        for (const auto & entry : entries)
        {
            if (source_uuid_set.contains(entry.source_part_uuid))
            {
                affected.push_back(ann_index_part_uuid);
                break;
            }
        }
    }
    ...
    const std::unordered_map<UUID, std::vector<CoverageEntry>> & coverage_by_ann_index_part_uuid)
{
    ...
    result.obsolete_coverage.affected_ann_index_part_uuids
        = collectAffectedANNIndexPartUuids(best_it->second, coverage_by_ann_index_part_uuid);
    ...
    Predicate && entry_can_precede)
{
    ...
    for (const auto & [ann_index_part_uuid, entries] : coverage_by_ann_index_part_uuid)
    {
        for (const auto & entry : entries)
        {
            ...
            if (selected_set.insert(ann_index_part_uuid).second)
                selected.push_back(ann_index_part_uuid);
        }
    }
...
bool tryBuildMergeLineageRemap(
    ...
    const std::vector<UUID> & delta_out,
    bool ann_index_snapshot_non_empty,
    const std::unordered_map<UUID, std::vector<CoverageEntry>> & coverage_by_ann_index_part_uuid,
    ReconcileResult & result)
{
    if (delta_in.size() != 1 || delta_out.empty() || !ann_index_snapshot_non_empty)
        return false;
    ...
    auto selected_ann_index_part_uuids = collectFullyCoveringLineageMiParts(
        delta_out,
        new_source_part,
        coverage_by_ann_index_part_uuid,
        entryCanPrecedeMergePart);
    if (selected_ann_index_part_uuids.empty())
        return false;

    result.candidate_kind = ReconcileCandidateKind::RemapLineage;
    result.remap_kind = ANNIndexRemapKind::MergeLineage;
    result.remap_lineage.remap_kind = ANNIndexRemapKind::MergeLineage;
    result.remap_lineage.old_ann_index_part_uuids = std::move(selected_ann_index_part_uuids);
...
bool tryBuildMutationLineageRemap(
    ...
    ReconcileResult & result)
{
    if (delta_in.size() != 1 || delta_out.size() != 1 || !ann_index_snapshot_non_empty)
        return false;
    ...
    result.candidate_kind = ReconcileCandidateKind::RemapLineage;
    result.remap_kind = ANNIndexRemapKind::MutationLineage;
    result.remap_lineage.remap_kind = ANNIndexRemapKind::MutationLineage;
    result.remap_lineage.old_ann_index_part_uuids = std::move(selected_ann_index_part_uuids);
    result.remap_lineage.old_source_part_uuids = delta_out;
    result.remap_lineage.new_source_part = new_source_part.part;
    result.remap_lineage.new_source_part_uuid = new_source_part.uuid;
    ...
    const std::unordered_set<UUID> & coverage)
{
    ...
    else if (ann_index_snapshot_non_empty && !result.delta_out.empty())
    {
        result.candidate_kind = ReconcileCandidateKind::ObsoleteCoverage;
        result.remap_kind = ANNIndexRemapKind::ObsoleteCoverageCleanup;
    ...
    const std::unordered_set<UUID> & coverage)
{
    ...
    auto & entries = coverage_by_ann_index_part_uuid[UUID{}];
    for (const auto & uuid : coverage)
    {
        CoverageEntry entry;
    ...
    return run(source_snapshot, ann_index_snapshot, coverage_by_ann_index_part_uuid);
}
    ...
    const std::unordered_map<UUID, std::vector<CoverageEntry>> & coverage_by_ann_index_part_uuid)
{
    ...
    const auto coverage_by_source_uuid = buildCoverageBySourceUuid(coverage_by_ann_index_part_uuid);
    ...
    if (delta_in_views.size() == 1 && !result.delta_out.empty())
    {
        ...
        result.rebuild_source_part.affected_ann_index_part_uuids
            = collectAffectedANNIndexPartUuids(result.delta_out, coverage_by_ann_index_part_uuid);
    ...
    else if (!result.delta_out.empty() && ann_index_snapshot_non_empty)
    {
        result.candidate_kind = ReconcileCandidateKind::ObsoleteCoverage;
        result.remap_kind = ANNIndexRemapKind::ObsoleteCoverageCleanup;
        result.obsolete_coverage.obsolete_source_part_uuids = result.delta_out;
        ...
        restrictObsoleteCoverageToSingleSourcePartition(result, coverage_by_source_uuid, coverage_by_ann_index_part_uuid);
    }
    ...
    const std::unordered_map<UUID, std::vector<CoverageEntry>> & coverage_by_ann_index_part_uuid)
{
    auto result = runOnPartViews(
        ...
        !ann_index_snapshot.empty(),
        coverage_by_ann_index_part_uuid);

    std::unordered_map<UUID, MergeTreeData::DataPartPtr> ann_index_parts_by_uuid;
    ann_index_parts_by_uuid.reserve(ann_index_snapshot.size());
    for (const auto & part : ann_index_snapshot)
        if (part)
            ann_index_parts_by_uuid.emplace(part->uuid, part);

    auto collect_parts = [&](const std::vector<UUID> & uuids)
    {
        MergeTreeData::DataPartsVector parts;
        parts.reserve(uuids.size());
        for (const auto & ann_index_part_uuid : uuids)
        {
            auto it = ann_index_parts_by_uuid.find(ann_index_part_uuid);
            if (it != ann_index_parts_by_uuid.end())
                parts.push_back(it->second);
        }
        return parts;
    };

    result.remap_lineage.old_ann_index_parts
        = collect_parts(result.remap_lineage.old_ann_index_part_uuids);
    result.obsolete_coverage.affected_ann_index_parts
        = collect_parts(result.obsolete_coverage.affected_ann_index_part_uuids);
    result.rebuild_source_part.affected_ann_index_parts
        = collect_parts(result.rebuild_source_part.affected_ann_index_part_uuids);
