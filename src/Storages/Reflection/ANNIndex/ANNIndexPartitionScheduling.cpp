...
bool compactRebuildPartitionConditionMet(
    ...
    const std::vector<PartIdentityView> & ann_index_parts,
    const std::unordered_set<UUID> & covered_source_uuids)
{
    if (ann_index_parts.empty() || source_parts.empty())
        return false;

    const String & physical_partition_id = ann_index_parts.front().partition_id;
    ...
    for (const auto & part : source_parts)
    {
        if (!covered_source_uuids.contains(part.uuid))
            return false;
        if (getANNIndexPhysicalPartitionId(part.partition_id) != physical_partition_id)
            return false;
    }
