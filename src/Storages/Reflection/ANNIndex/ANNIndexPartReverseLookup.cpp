...
ANNIndexPartReverseLookup::LocatorEntry ANNIndexPartReverseLookup::liveLocatorEntry(UInt32 part_uuid_id, UInt64 part_offset)
{
    return {part_uuid_id, part_offset};
}


ANNIndexPartReverseLookup::LocatorEntry ANNIndexPartReverseLookup::tombstoneLocatorEntry()
{
    return {TOMBSTONE_PART_UUID_ID, 0};
}


ANNIndexPartReverseLookup::LocatorEntry ANNIndexPartReverseLookup::readLocatorEntry(ReadBuffer & in)
{
    LocatorEntry entry;
...
void ANNIndexPartReverseLookup::writeLocatorEntry(const LocatorEntry & entry, WriteBuffer & out)
{
...
void ANNIndexPartReverseLookup::addLocatorHeaderFields(Poco::JSON::Object & header)
{
    header.set("tombstone_part_uuid_id", static_cast<UInt64>(TOMBSTONE_PART_UUID_ID));
}


void ANNIndexPartReverseLookup::validateLocatorHeader(const Poco::JSON::Object & header)
{
    if (!header.has("part_uuid_table"))
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: header.json missing part_uuid_table");

    if (!header.has("tombstone_part_uuid_id"))
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: header.json missing tombstone_part_uuid_id");
    ...
    if (tombstone_part_uuid_id != TOMBSTONE_PART_UUID_ID)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: unsupported tombstone_part_uuid_id {}, expected {}",
            tombstone_part_uuid_id, TOMBSTONE_PART_UUID_ID);
}


ANNIndexPartReverseLookup::ANNIndexPartReverseLookup(const IDataPartStorage & storage_)
    : storage(storage_)
{
    if (!storage.existsFile("header.json"))
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: header.json missing in materialized-index-part {}", storage.getRelativePath());
    ...
    if (!obj)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: header.json is not a JSON object");
    ...
    if (!uuid_arr)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: part_uuid_table is not an array");
    ...
    for (size_t i = 0; i < uuid_arr->size(); ++i)
    {
        ...
        if (!tryParse(uuid, uuid_text))
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                "ANNIndexPartReverseLookup: cannot parse part_uuid_table[{}] as UUID: {}",
                i, uuid_text);
    ...
    if (boundaries_var.isEmpty())
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: header.json missing segment_boundaries");
    const auto & boundaries_arr = boundaries_var.extract<Poco::JSON::Array::Ptr>();
    if (!boundaries_arr)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: segment_boundaries is not an array");
    ...
    if (segment_boundaries.empty() || segment_boundaries.front() != 0)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: segment_boundaries must start at 0");

    total_rows = segment_boundaries.back();
}


size_t ANNIndexPartReverseLookup::segmentIndexFor(UInt64 hit_id) const
{
...
const ANNIndexPartReverseLookup::LocatorEntry & ANNIndexPartReverseLookup::getLocatorEntry(
...
void ANNIndexPartReverseLookup::loadLocatorPage(size_t segment_index, UInt64 page_start_row, LocatorPage & page)
{
    const String segment_path = fmt::format("locator_{}.bin", segment_index);
    if (!storage.existsFile(segment_path))
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: {} missing", segment_path);
    ...
    if (page_start_row >= expected_rows)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: page_start_row {} >= expected_rows {} for {}",
    ...
    for (UInt64 i = 0; i < rows_to_read; ++i)
    {
        if (reader->eof())
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                "ANNIndexPartReverseLookup: {} truncated at row {} (expected {})",
...
ANNIndexPartReverseLookup::SourceRow ANNIndexPartReverseLookup::lookup(UInt64 hit_id)
{
    if (hit_id >= total_rows)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: hit_id {} >= total_rows {}", hit_id, total_rows);
    ...
    if (entry.part_uuid_id >= uuid_table.size())
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "ANNIndexPartReverseLookup: part_uuid_id {} >= part_uuid_table.size {}",
