...
/// index. Mirrors BuildTask: a four-state state machine where `prepare`
/// constructs the mid-layer MaterializedIndexRemapTask, `executeStep` drives
/// its stages, and `finish` commits the produced parts atomically through a
/// MergeTreeData::Transaction (which also marks superseded mi-parts Outdated).
...
private:
    ...
    std::unique_ptr<MaterializedIndexRemapTask> remap_mi_part_task;
