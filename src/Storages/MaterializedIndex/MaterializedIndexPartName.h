...
struct MaterializedIndexSourcePartitionRange
{
...
inline constexpr auto MATERIALIZED_INDEX_SOURCE_PARTITION_ID_COLUMN = "_source_partition_id";

inline const Block & getMaterializedIndexPartitionKeySampleBlock()
{
    static const Block sample_block
    {
        ColumnWithTypeAndName{
            ...
            MATERIALIZED_INDEX_SOURCE_PARTITION_ID_COLUMN}
    };
    return sample_block;
}

inline String getMaterializedIndexSourcePartitionHash(const String & source_partition_id)
{
    SipHash hash;
...
inline String getMaterializedIndexPhysicalPartitionId(const String & source_partition_id)
{
    MergeTreePartition partition(Row{source_partition_id});
    return partition.getID(getMaterializedIndexPartitionKeySampleBlock());
}

inline MergeTreePartInfo markAsMaterializedIndexPartInfo(MergeTreePartInfo part_info)
{
...
MaterializedIndexSourcePartitionRange getMaterializedIndexSourcePartitionRange(const DataParts & source_parts)
{
    std::optional<MaterializedIndexSourcePartitionRange> range;
    ...
    for (const auto & part : source_parts)
    {
        ...
        if (!range)
        {
            ...
            range->source_partition_hash = getMaterializedIndexSourcePartitionHash(source_partition_id);
...
template <typename DataParts>
String makeMaterializedIndexBuildPartNameFromSourceParts(
    const DataParts & source_parts,
    MergeTreeDataFormatVersion format_version)
{
    const auto range = getMaterializedIndexSourcePartitionRange(source_parts);
    MergeTreePartInfo part_info(
        getMaterializedIndexPhysicalPartitionId(range.source_partition_id),
        range.min_block,
        range.max_block,
        /*level_=*/0,
        range.mutation);
    ...
    MergeTreeDataFormatVersion format_version)
{
    ...
    return markAsMaterializedIndexPartInfo(*compact_info).getPartNameAndCheckFormat(format_version);
}
