...
/// Read the `part_uuid_dict.bin` of an old mi-part into a vector indexed by
...
/// Parse a part name of the form `mi-<partition>_<min>_<max>_<level>` (or
...
/// Derive a `tmp_mi_remap_<new_part_name>` storage that lives on the same
    ...
    const String & new_part_name)
{
    ...
    const String tmp_dir_name = String{MaterializedIndexRemapTask::TEMP_DIRECTORY_PREFIX} + new_part_name;
...
struct MaterializedIndexRemapTask::PlanAffectedSegmentsStage : public IStage
{
    ...
    bool execute() override
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            /// N=M derivation: one new mi-part per old, `level` bumped by one.
...
struct MaterializedIndexRemapTask::DeriveHardlinksStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr, StageRuntimeContextPtr global) override
    {
        global_ctx = std::static_pointer_cast<GlobalRuntimeContext>(global);
...
struct MaterializedIndexRemapTask::RewriteMutableSegmentsStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr, StageRuntimeContextPtr global) override
    {
        global_ctx = std::static_pointer_cast<GlobalRuntimeContext>(global);
    ...
    bool execute() override
    {
        ...
        while (!reader->eof())
        {
            ...
            writeBinaryLittleEndian(out_dict_id, *writer);
            writeBinaryLittleEndian(out_part_offset, *writer);
...
struct MaterializedIndexRemapTask::FinalizeMetadataStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr, StageRuntimeContextPtr global) override
    {
        global_ctx = std::static_pointer_cast<GlobalRuntimeContext>(global);
...
MaterializedIndexRemapTask::Stages MaterializedIndexRemapTask::makeStages()
{
    return {
...
MaterializedIndexRemapTask::~MaterializedIndexRemapTask()
{
    ...
    try
    {
        global_ctx->promise.set_exception(std::make_exception_ptr(
            Exception(ErrorCodes::LOGICAL_ERROR, "MaterializedIndexRemapTask destroyed before completion")));
    }
...
bool MaterializedIndexRemapTask::execute()
try
{
...
std::future<std::vector<MergeTreeData::MutableDataPartPtr>> MaterializedIndexRemapTask::getFuture()
{
    return global_ctx->promise.get_future();
}


std::vector<MergeTreeData::MutableDataPartPtr> MaterializedIndexRemapTask::getUnfinishedParts()
{
