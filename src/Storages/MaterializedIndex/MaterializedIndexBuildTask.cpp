...
{
    extern const MergeTreeSettingsUInt64 mi_segment_size_rows;
}
...
/// Dictionary widths (see D-16). UInt32 upper bound on dict size is
/// 4.3 billion distinct keys; if a build produces more the writer throws
/// LOGICAL_ERROR so the caller notices rather than silently truncating.
constexpr UInt32 MAX_DICT_ID = std::numeric_limits<UInt32>::max();
...
struct MaterializedIndexBuildTask::ReadColumnsWriteLocatorAndPrepareStage : public IStage
{
    ...
    void openPipelineForCurrentPart()
    {
        ...
        createReadFromPartStep(
            ...
            *global_ctx->source_storage,
            ...
            /*prefetch=*/false,
            global_ctx->context,
            getLogger("MaterializedIndexBuildTask"));
    ...
    void openStableLayerSegmentIfNeeded()
    {
        ...
        /// Segment index = number of boundaries accumulated so far (because
        /// boundaries are pushed at the *end* of each segment in the current
        /// scheme, and the starting boundary 0 is appended in finalizeStage).
        const size_t segment_index = global_ctx->segment_boundaries_buffer.size();
    ...
    UInt64 getSegmentSizeRows() const
    {
        ...
        if (global_ctx->storage)
            return (*global_ctx->storage->getSettings())[MergeTreeSetting::mi_segment_size_rows];
...
struct MaterializedIndexBuildTask::WriteMutableLayerStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr local, StageRuntimeContextPtr global) override
    {
        local_ctx = std::move(local);
...
struct MaterializedIndexBuildTask::BuildAlgorithmStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr local, StageRuntimeContextPtr global) override
    {
        local_ctx = std::move(local);
    ...
    bool execute() override
    {
        ...
        if (global_ctx->algorithm)
        {
            ...
            {
                tryLogCurrentException(getLogger("MaterializedIndexBuildTask"), __PRETTY_FUNCTION__);
                throw;
            }
        }
        return false;
...
struct MaterializedIndexBuildTask::FinishAlgorithmStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr local, StageRuntimeContextPtr global) override
    {
        local_ctx = std::move(local);
...
struct MaterializedIndexBuildTask::CleanupIntermediateStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr local, StageRuntimeContextPtr global) override
    {
        local_ctx = std::move(local);
...
struct MaterializedIndexBuildTask::FinalizeMetadataStage : public IStage
{
    void setRuntimeContext(StageRuntimeContextPtr local, StageRuntimeContextPtr global) override
    {
        local_ctx = std::move(local);
...
MaterializedIndexBuildTask::Stages MaterializedIndexBuildTask::makeStages()
{
    return {
...
MaterializedIndexBuildTask::~MaterializedIndexBuildTask()
{
    ...
    try
    {
        global_ctx->promise.set_exception(std::make_exception_ptr(
            Exception(ErrorCodes::LOGICAL_ERROR, "MaterializedIndexBuildTask destroyed before completion")));
    }
...
bool MaterializedIndexBuildTask::execute()
try
{
...
std::future<MergeTreeData::MutableDataPartPtr> MaterializedIndexBuildTask::getFuture()
{
    return global_ctx->promise.get_future();
}


MergeTreeData::MutableDataPartPtr MaterializedIndexBuildTask::getUnfinishedPart()
{
