...
NamesAndTypesList getMaterializedIndexColumns(const MergeTreeData * inner_storage)
{
    if (inner_storage)
    {
...
int32_t getMaterializedIndexMetadataVersion(const MergeTreeData * inner_storage)
{
    if (!inner_storage)
        return 0;
...
String getMaterializedIndexSourcePartitionMinMaxFileName(
    const IDataPartStorage & part_storage,
    const MergeTreeData * inner_storage)
{
    ...
    if (inner_storage)
        ...
        column_file_name = escapeForFileName(MATERIALIZED_INDEX_SOURCE_PARTITION_ID_COLUMN);

    return "minmax_" + column_file_name + ".idx";
}

void writeMaterializedIndexPartitionMetadata(
    IDataPartStorage & part_storage,
    const MergeTreeData * inner_storage,
    ...
    const WriteSettings & write_settings)
{
    ...
    const String minmax_file_name = getMaterializedIndexSourcePartitionMinMaxFileName(part_storage, inner_storage);
...
MergeTreeDataPartChecksums writeMaterializedIndexPartMetadata(
    IDataPartStorage & part_storage,
    const MergeTreeData * inner_storage,
    UInt64 rows_count,
    const String & source_partition_id,
    UUID part_uuid)
{
    ...
    {
        getMaterializedIndexColumns(inner_storage).writeText(out);
    });
    ...
    writeMaterializedIndexPartitionMetadata(part_storage, inner_storage, source_partition_id, write_settings);
    ...
    {
        writeIntText(getMaterializedIndexMetadataVersion(inner_storage), out);
    });
