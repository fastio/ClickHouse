...
/// Matching algorithm shared by MergeTreeRangeReader's ANNIndex fill function
...
///
/// The helper is intentionally ANNIndex-only. The vector_similarity_index variant
/// keeps its inlined copy of the same algorithm because the two paths must remain
/// physically independent — see the design notes around `ann_index_search_results`.
///
/// Inputs:
///   row_offsets_from_index    rows returned by the ANNIndex search.
