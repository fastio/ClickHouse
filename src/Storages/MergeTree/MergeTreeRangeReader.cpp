...
void MergeTreeRangeReader::fillVirtualColumns(Columns & columns, ReadResult & result)
{
    ...
    else if (is_ann_index)
    {
        ColumnPtr part_offsets_auto_column = createPartOffsetColumn(result);
        fillDistanceColumnAndFilterForANNIndex(columns, result, part_offsets_auto_column);
    }
...
void MergeTreeRangeReader::fillDistanceColumnAndFilterForANNIndex(Columns & columns, ReadResult & /*result*/, ColumnPtr & part_offsets_auto_column)
{
    /// ANNIndex path: physically isolated from the vector path.
    /// The matching algorithm is delegated to a ANNIndex-only file-local helper
    /// (see `buildMatchingFilterAndDistanceColumnForANNIndex` below). The helper is
    ...
    const auto & read_hints = merge_tree_reader->data_part_info_for_read->getReadHints();
    const auto & offsets_and_distances = read_hints.ann_index_search_results.value();
    chassert(offsets_and_distances.distances.has_value());
