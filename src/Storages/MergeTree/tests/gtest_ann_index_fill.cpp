    ...
    const std::vector<UInt64> & part_offsets_vec)
{
    ...
    buildMatchingFilterAndDistanceColumnForANNIndex(
...
TEST(FillDistanceForANNIndex, Basic)
{
...
TEST(FillDistanceForANNIndex, OutOfOrderRows)
{
...
TEST(FillDistanceForANNIndex, EmptyRows)
{
...
TEST(FillDistanceForANNIndex, RowsBeyondRange)
{
...
TEST(FillDistanceForANNIndex, MissingDistances)
{
#ifdef NDEBUG
...
TEST(FillDistanceForANNIndex, LengthMismatch)
{
#ifdef NDEBUG
    GTEST_SKIP() << "Release build skips chassert-based death tests";
#else
...
TEST(FillDistanceForANNIndex, DistanceColumnTypeIsFloat32)
{
...
/// T7 — RangesInDataPartReadHints carries vector and ANNIndex results in two
...
/// ANNIndex SQL regression — here we only pin the data-shape contract that
...
TEST(RangeReader1196Branch, VectorWinsWhenBothPresent)
{
    ...
    hints.ann_index_search_results = NearestNeighbours{{1, 2}, std::vector<float>{0.5f, 0.6f}};
    EXPECT_FALSE(hints.vector_search_results.has_value());
    EXPECT_TRUE(hints.ann_index_search_results.has_value());
    ...
    EXPECT_TRUE(hints.ann_index_search_results.has_value());
    EXPECT_NE(hints.vector_search_results->rows, hints.ann_index_search_results->rows);
}

/// T8 — `RangesInDataPartReadHints` carries the vector and ANNIndex hints in
...
/// branch is exercised end-to-end by the ANNIndex SQL regression.
TEST(RangeReader1594Branch, MIFilterApplied)
{
    RangesInDataPartReadHints hints;
    const void * vec_addr = static_cast<const void *>(&hints.vector_search_results);
    const void * ann_index_addr = static_cast<const void *>(&hints.ann_index_search_results);
    EXPECT_NE(vec_addr, ann_index_addr);
}
