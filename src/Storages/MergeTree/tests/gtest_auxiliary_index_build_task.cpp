...
/// Minimal algorithm stub that satisfies IAuxiliaryIndexAlgorithm's pure
...
class BuildOnlyMockAlgorithm : public IAuxiliaryIndexAlgorithm
{
public:
    ...
    std::unique_ptr<IAuxiliaryIndexAlgorithm> cloneForBuild() const override
    {
        ...
        /// in these unit tests; the framework-side `AuxiliaryIndexBuildTask`
...
public:
    ...
    {
        throw std::runtime_error("injected AuxiliaryIndex build failure");
    }
};
...
TEST(AuxiliaryIndexBuildTaskTest, EmptyStagesStateMachineAdvances)
{
...
TEST(AuxiliaryIndexBuildTaskTest, SkeletonPromiseResolvesWithEmptyPart)
{
    BuildOnlyMockAlgorithm algorithm;
    ...
    /// The skeleton stage 6 leaves new_auxiliary_index_part null on purpose; the promise
...
TEST(AuxiliaryIndexBuildTaskTest, StageExceptionResolvesPromiseWithException)
{
...
TEST(AuxiliaryIndexBuildTaskTest, CancelIsIdempotent)
{
    BuildOnlyMockAlgorithm algorithm;
...
class AuxiliaryIndexBuildTaskStage6Test : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        disk = std::make_shared<DiskLocal>("auxiliary_index_build_test_disk", temp_dir->path());
        disk->createDirectories("output/part");
        volume = std::make_shared<SingleDiskVolume>("auxiliary_index_build_test_vol", disk, 0);
...
TEST_F(AuxiliaryIndexBuildTaskStage6Test, WritesStandardMetadataFiles)
{
    BuildOnlyMockAlgorithm algorithm;
...
TEST_F(AuxiliaryIndexBuildTaskStage6Test, PartFormatFromDiskDetectsAuxiliaryIndexLayout)
{
    ...
    EXPECT_EQ(part_type->getValue(), MergeTreeDataPartType::AuxiliaryIndex);
}


TEST_F(AuxiliaryIndexBuildTaskStage6Test, HeaderJsonCarriesAlgorithmIdentityAndVersion)
{
    BuildOnlyMockAlgorithm algorithm;
    ...
    EXPECT_EQ(obj->getValue<UInt64>("tombstone_part_uuid_id"), AuxiliaryIndexPartReverseLookup::TOMBSTONE_PART_UUID_ID);
    EXPECT_EQ(obj->getValue<UInt64>("segment_count"), 0U);
    EXPECT_EQ(obj->getValue<UInt64>("coverage_source_part_count"), 0U);
}


TEST_F(AuxiliaryIndexBuildTaskStage6Test, DoesNotWriteTxnVersionMetadata)
{
    BuildOnlyMockAlgorithm algorithm;
...
TEST_F(AuxiliaryIndexBuildTaskStage6Test, CoverageJsonEmptyForZeroSourceParts)
{
    BuildOnlyMockAlgorithm algorithm;
...
TEST_F(AuxiliaryIndexBuildTaskStage6Test, ChecksumsTxtCoversAuxiliaryIndexEnvelope)
{
    BuildOnlyMockAlgorithm algorithm;
