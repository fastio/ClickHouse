...
TEST(FillDistanceForMaterializedIndex, Basic)
{
...
TEST(FillDistanceForMaterializedIndex, OutOfOrderRows)
{
...
TEST(FillDistanceForMaterializedIndex, EmptyRows)
{
...
TEST(FillDistanceForMaterializedIndex, RowsBeyondRange)
{
...
TEST(FillDistanceForMaterializedIndex, MissingDistances)
{
#ifdef NDEBUG
...
TEST(FillDistanceForMaterializedIndex, LengthMismatch)
{
#ifdef NDEBUG
    GTEST_SKIP() << "Release build skips chassert-based death tests";
#else
...
TEST(FillDistanceForMaterializedIndex, DistanceColumnTypeIsFloat32)
{
...
/// T7 — RangesInDataPartReadHints carries vector and MaterializedIndex results in two
...
/// MaterializedIndex SQL regression — here we only pin the data-shape contract that
...
/// branch is exercised end-to-end by the MaterializedIndex SQL regression.
