...
class RemapOnlyMockAlgorithm : public IANNIndexAlgorithm
{
public:
...
TEST(ANNIndexRemapTaskImplTest, SkeletonPromiseResolvesWithEmptyVector)
{
    ...
    /// `new_ann_index_parts` (Pack 2 lands that logic).
...
/// stage-6 fixture: DiskLocal-backed storages plus empty affected_ann_index_parts
...
protected:
    void SetUp() override
    {
        ...
        disk = std::make_shared<DiskLocal>("ann_index_remap_test_disk", temp_dir->path());
...
TEST_F(ANNIndexRemapTaskImplStage4Test, EndToEndZeroAffectedEmitsEmptyVector)
{
    ...
    /// Zero affected input -> stage 1 produces no new_ann_index_parts; stage 4
...
TEST_F(ANNIndexRemapTaskImplStage4Test, Stage4WritesNoMetadataFilesForEmptyRemap)
{
    ...
    /// With no new_ann_index_parts, stage 4 never opens any on-disk writers. The
...
TEST_F(ANNIndexRemapTaskImplStage4Test, ZeroAlgorithmCalls)
{
    ...
    /// The mock algorithm is conceptually owned by `ReflectionANNIndex`
