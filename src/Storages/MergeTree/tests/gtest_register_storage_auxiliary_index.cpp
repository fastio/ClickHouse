...
/// `registerStorageANN.cpp` for test-only access.
...
DB::ContextMutablePtr makeTestContext(bool gate_value)
{
    ...
    context->setSetting("allow_experimental_auxiliary_index", gate_value);
    return context;
}
...
TEST(GatedFactoryTest, RejectsWithoutGate)
{
    ...
    try
    {
        DB::checkAuxiliaryIndexExperimentalGate(context, DB::LoadingStrictnessLevel::CREATE);
...
TEST(GatedFactoryTest, AcceptsWithGate)
{
    ...
    EXPECT_NO_THROW(DB::checkAuxiliaryIndexExperimentalGate(context, DB::LoadingStrictnessLevel::CREATE));
}


/// `ATTACH` and metadata-loading paths must not be gated, otherwise persisted
/// `AuxiliaryIndex` tables can prevent server startup after restart.
TEST(GatedFactoryTest, AttachModeBypassesGate)
{
    auto context = makeTestContext(/*gate_value=*/false);

    EXPECT_NO_THROW(DB::checkAuxiliaryIndexExperimentalGate(context, DB::LoadingStrictnessLevel::ATTACH));
}
