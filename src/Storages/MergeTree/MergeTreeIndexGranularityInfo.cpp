...
{
    /// AuxiliaryIndex parts carry no mark files at all, so the
    ...
    if (part_type == MergeTreeDataPartType::AuxiliaryIndex)
        return;
...
std::string MarkType::getFileExtension() const
{
    ...
    switch (part_type)
    {
        ...
        case MergeTreeDataPartType::AuxiliaryIndex:
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                "AuxiliaryIndex parts have no mark files; getFileExtension is not applicable");
