...
String getANNIndexKeeperGuardPath(
    ...
    const String & payload)
{
    ...
    hash.update(payload);
    return fs::path(zookeeper_path) / "ann_index" / guard_group / getHexUIntLowercase(hash.get128());
}

void addANNIndexKeeperChecksToOps(
    Coordination::Requests & ops,
    const StorageReplicatedMergeTree::ANNIndexKeeperChecks & keeper_checks)
{
    ...
    const ANNIndexKeeperChecks & keeper_checks)
{
    ...
    try
    {
        ...
        addANNIndexKeeperChecksToOps(ops, keeper_checks);
...
bool StorageReplicatedMergeTree::tryAcquireANNIndexLeaderLease(const String & payload, String & lease_path)
{
    auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::tryAcquireANNIndexLeaderLease");

    auto zookeeper = getZooKeeper();
    lease_path = zookeeper_path + "/ann_index/leader";
    ...
    const String guard_path = getANNIndexKeeperGuardPath(zookeeper_path, "leader_guards", lease_path, payload);
...
void StorageReplicatedMergeTree::assertANNIndexLeaderLease(
    const String & lease_path,
    const String & expected_payload,
    ANNIndexKeeperChecks * keeper_checks) const
{
    if (lease_path.empty() || expected_payload.empty())
        throw Exception(ErrorCodes::ABORTED, "Lost ANNIndex leader lease before commit");

    auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::assertANNIndexLeaderLease");

    auto zookeeper = getZooKeeper();
    String payload;
    Coordination::Stat stat;
    if (!zookeeper->tryGet(lease_path, payload, &stat) || payload != expected_payload)
        throw Exception(ErrorCodes::ABORTED, "Lost ANNIndex leader lease before commit: {}", lease_path);

    const String guard_path = getANNIndexKeeperGuardPath(zookeeper_path, "leader_guards", lease_path, expected_payload);
    ...
    if (!zookeeper->tryGet(guard_path, guard_payload, &guard_stat) || guard_payload != expected_payload)
        throw Exception(ErrorCodes::ABORTED, "Lost ANNIndex leader lease guard before commit: {}", guard_path);
...
void StorageReplicatedMergeTree::releaseANNIndexLeaderLease(const String & lease_path, const String & expected_payload) noexcept
{
    if (lease_path.empty() || expected_payload.empty())
        return;
    ...
    try
    {
        auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::releaseANNIndexLeaderLease");
        auto zookeeper = getZooKeeper();
        String payload;
        Coordination::Stat stat;
        if (zookeeper->tryGet(lease_path, payload, &stat) && payload == expected_payload)
    ...
    {
        tryLogCurrentException(log, "Failed to release ANNIndex leader lease");
    }
}

bool StorageReplicatedMergeTree::tryReserveANNIndexTask(
    const String & task_key,
    const String & payload,
    String & lock_path)
{
    auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::tryReserveANNIndexTask");

    auto zookeeper = getZooKeeper();
    lock_path = zookeeper_path + "/ann_index/locks/" + task_key;
    ...
    const String guard_path = getANNIndexKeeperGuardPath(zookeeper_path, "lock_guards", lock_path, payload);
    zookeeper->createAncestors(guard_path);
    auto guard_code = zookeeper->tryCreate(guard_path, payload, zkutil::CreateMode::Ephemeral);
    if (guard_code == Coordination::Error::ZOK)
        return true;
...
void StorageReplicatedMergeTree::assertANNIndexTaskReservation(
    const String & lock_path,
    const String & expected_payload,
    ANNIndexKeeperChecks * keeper_checks) const
{
    if (lock_path.empty() || expected_payload.empty())
        throw Exception(ErrorCodes::ABORTED, "Lost ANNIndex Keeper task reservation before commit");

    auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::assertANNIndexTaskReservation");

    auto zookeeper = getZooKeeper();
    String payload;
    Coordination::Stat stat;
    if (!zookeeper->tryGet(lock_path, payload, &stat) || payload != expected_payload)
        throw Exception(ErrorCodes::ABORTED, "Lost ANNIndex Keeper task reservation before commit: {}", lock_path);

    const String guard_path = getANNIndexKeeperGuardPath(zookeeper_path, "lock_guards", lock_path, expected_payload);
    String guard_payload;
    Coordination::Stat guard_stat;
    if (!zookeeper->tryGet(guard_path, guard_payload, &guard_stat) || guard_payload != expected_payload)
        throw Exception(ErrorCodes::ABORTED, "Lost ANNIndex Keeper task reservation guard before commit: {}", guard_path);
...
void StorageReplicatedMergeTree::releaseANNIndexTask(const String & lock_path, const String & expected_payload) noexcept
{
    if (lock_path.empty() || expected_payload.empty())
        return;
    ...
    try
    {
        auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::releaseANNIndexTask");
        auto zookeeper = getZooKeeper();
        String payload;
        Coordination::Stat stat;
        if (zookeeper->tryGet(lock_path, payload, &stat) && payload == expected_payload)
    ...
    {
        tryLogCurrentException(log, "Failed to release ANNIndex Keeper task reservation");
    }
}
