...
TEST(CoverageMapTest, ReplaceAllThenReadUuids)
{
    ...
    CoverageMap m;
    UUID auxiliary_index_a = mkUuid(0xA);
    UUID auxiliary_index_b = mkUuid(0xB);
    UUID u1 = mkUuid(1);
    UUID u2 = mkUuid(2);
    UUID u3 = mkUuid(3);
    UUID u4 = mkUuid(4);
    ...
    snapshot.emplace_back(auxiliary_index_a, std::vector<CoverageEntry>{mkEntry(u1, 100), mkEntry(u2, 200), mkEntry(u3, 300)});
    snapshot.emplace_back(auxiliary_index_b, std::vector<CoverageEntry>{mkEntry(u2, 200), mkEntry(u3, 300), mkEntry(u4, 400)});
...
TEST(CoverageMapTest, AppendFromBuildAccumulates)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 10), mkEntry(u2, 20)});
    m.appendFromBuild(auxiliary_index_b, {mkEntry(u3, 30)});
...
TEST(CoverageMapTest, ConcurrentBuildCommitsMergeDisjointCoverage)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 10), mkEntry(u2, 20)});
    m.appendFromBuild(auxiliary_index_b, {mkEntry(u3, 30), mkEntry(u4, 40)});

    auto by_auxiliary_index_part = m.coverageEntriesByMiPartUuid();
    ASSERT_TRUE(by_auxiliary_index_part.contains(auxiliary_index_a));
    ASSERT_TRUE(by_auxiliary_index_part.contains(auxiliary_index_b));
    EXPECT_EQ(by_auxiliary_index_part.at(auxiliary_index_a).size(), 2u);
    EXPECT_EQ(by_auxiliary_index_part.at(auxiliary_index_b).size(), 2u);
...
TEST(CoverageMapTest, EntriesBySourceUuidPreservesPartInfo)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntryWithPartInfo(u1, 10, "p", 1, 3, 2, 0)});
...
TEST(CoverageMapTest, EntriesByMiPartAndReverseLookup)
{
    ...
    ASSERT_TRUE(by_auxiliary_index_part.contains(auxiliary_index_b));
    EXPECT_EQ(by_auxiliary_index_part.at(auxiliary_index_a).size(), 2u);
    EXPECT_EQ(by_auxiliary_index_part.at(auxiliary_index_b).size(), 1u);

    auto affected = m.miPartUuidsCoveringAnySourceUuid({u2, u3});
    EXPECT_EQ(affected.size(), 2u);
    EXPECT_TRUE(affected.contains(auxiliary_index_a));
    EXPECT_TRUE(affected.contains(auxiliary_index_b));
}
...
TEST(CoverageMapTest, ApplyRemapReplacesOldMiPart)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 10), mkEntry(u2, 20)});
    m.applyRemap(auxiliary_index_b, auxiliary_index_a, {mkEntry(u1, 10), mkEntry(u2, 20), mkEntry(u3, 30)}, {});
    ...
    /// Drop auxiliary_index_a again — it must be a no-op since `applyRemap` already retired it.
    m.dropMiPart(auxiliary_index_a);
...
TEST(CoverageMapTest, ApplyRemapBatchReplacesOldMiPartsAtomically)
{
    ...
    UUID auxiliary_index_a = mkUuid(0xA);
    UUID auxiliary_index_b = mkUuid(0xB);
    UUID auxiliary_index_c = mkUuid(0xC);
    UUID auxiliary_index_d = mkUuid(0xD);
    UUID u1 = mkUuid(1);
    UUID u2 = mkUuid(2);
    UUID u3 = mkUuid(3);
    UUID u4 = mkUuid(4);

    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 10)});
    m.appendFromBuild(auxiliary_index_b, {mkEntry(u2, 20)});
    m.applyRemapBatch({
        {auxiliary_index_c, auxiliary_index_a, {mkEntry(u1, 10), mkEntry(u3, 30)}},
        {auxiliary_index_d, auxiliary_index_b, {mkEntry(u2, 20), mkEntry(u4, 40)}},
    });

    auto by_auxiliary_index_part = m.coverageEntriesByMiPartUuid();
    EXPECT_FALSE(by_auxiliary_index_part.contains(auxiliary_index_a));
    EXPECT_FALSE(by_auxiliary_index_part.contains(auxiliary_index_b));
    ASSERT_TRUE(by_auxiliary_index_part.contains(auxiliary_index_c));
    ASSERT_TRUE(by_auxiliary_index_part.contains(auxiliary_index_d));
    EXPECT_EQ(by_auxiliary_index_part.at(auxiliary_index_c).size(), 2u);
    EXPECT_EQ(by_auxiliary_index_part.at(auxiliary_index_d).size(), 2u);
...
TEST(CoverageMapTest, ApplyCompactReplacesSeveralOldMiParts)
{
    ...
    m.applyCompact(
        auxiliary_index_c,
        {auxiliary_index_a, auxiliary_index_b},
    ...
    m.dropMiPart(auxiliary_index_a);
    m.dropMiPart(auxiliary_index_b);
    EXPECT_EQ(m.coveredSourceUuids().size(), 3u);
}
...
TEST(CoverageMapTest, DropMiPartIdempotent)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 1)});
    m.dropMiPart(auxiliary_index_a);
    EXPECT_NO_THROW(m.dropMiPart(auxiliary_index_a));
...
TEST(CoverageMapTest, IsFullyCoveringSuperset)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 1), mkEntry(u2, 2), mkEntry(u3, 3)});
...
TEST(CoverageMapTest, IsFullyCoveringMissingFails)
{
    ...
    m.appendFromBuild(auxiliary_index_a, {mkEntry(u1, 1), mkEntry(u2, 2)});
