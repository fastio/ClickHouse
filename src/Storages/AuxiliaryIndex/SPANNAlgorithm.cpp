#include <Storages/AuxiliaryIndex/SPANNAlgorithm.h>
...
{
    extern const Event AuxiliaryIndexSPANNBuildStarted;
    extern const Event AuxiliaryIndexSPANNBuildFinished;
    extern const Event AuxiliaryIndexSPANNBuildFailed;
    extern const Event AuxiliaryIndexSPANNSearchStarted;
    extern const Event AuxiliaryIndexSPANNSearchFinished;
    extern const Event AuxiliaryIndexSPANNSearchFailed;
}
...
std::unique_ptr<IAuxiliaryIndexAlgorithm> SPANNAlgorithm::cloneForBuild() const
{
...
void SPANNAlgorithm::initialize(const AuxiliaryIndexContext & /*ctx*/)
{
    initialized = true;
}
...
{
    ProfileEvents::increment(ProfileEvents::AuxiliaryIndexSPANNSearchStarted);
    bool search_finished = false;
    scope_guard failed_search_guard = [&search_finished]
    {
        if (!search_finished)
            ProfileEvents::increment(ProfileEvents::AuxiliaryIndexSPANNSearchFailed);
    };
    ...
    ProfileEvents::increment(ProfileEvents::AuxiliaryIndexSPANNSearchFinished);
...
void SPANNAlgorithm::prepareBuild(const AlgorithmBuildContext & ctx, const Block & indexed_columns_batch)
{
    ...
    if (!build_started)
    {
        ProfileEvents::increment(ProfileEvents::AuxiliaryIndexSPANNBuildStarted);
        build_started = true;
...
void SPANNAlgorithm::buildAlgorithmPrivate(const AlgorithmBuildContext & ctx)
{
    ...
    {
        ProfileEvents::increment(ProfileEvents::AuxiliaryIndexSPANNBuildFailed);
        throw;
    }

    ProfileEvents::increment(ProfileEvents::AuxiliaryIndexSPANNBuildFinished);
}
