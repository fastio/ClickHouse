...
#include <Storages/AuxiliaryIndex/BuildTask.h>
#include <Storages/AuxiliaryIndex/AuxiliaryIndexPartCommitter.h>
...
void assertReplacedPartsMatchInput(
    ...
    const MergeTreeData::DataPartsVector & input_auxiliary_index_parts,
    const String & new_part_name)
{
    auto replaced_names = collectPartNames(replaced_parts);
    auto input_names = collectPartNames(input_auxiliary_index_parts);
    ...
    if (replaced_names != input_names)
        throw Exception(
            ...
            "AuxiliaryIndex compact part {} replaced unexpected parts: [{}] instead of [{}]",
            new_part_name,
...
AuxiliaryIndexCompactTask::AuxiliaryIndexCompactTask(
    StorageANN & storage_,
    ...
    , input_auxiliary_index_parts(std::move(input_auxiliary_index_parts_))
...
{
    for (const auto & part : input_auxiliary_index_parts)
        if (part)
            priority.value += part->getBytesOnDisk();
}

AuxiliaryIndexCompactTask::~AuxiliaryIndexCompactTask() = default;

StorageID AuxiliaryIndexCompactTask::getStorageID() const
{
    return storage_ref.getStorageID();
}

String AuxiliaryIndexCompactTask::getQueryId() const
{
...
void AuxiliaryIndexCompactTask::writeTaskLog(
    ...
    UInt64 bytes_added) const
{
    ...
    auto log = context->getAuxiliaryIndexLog();
    if (!log)
        return;

    AuxiliaryIndexLogElement element;
    element.type = type;
    ...
    element.input_auxiliary_index_parts = collectPartNames(input_auxiliary_index_parts);
...
void AuxiliaryIndexCompactTask::onCompleted()
{
    if (task_result_callback)
        task_result_callback(true);
}

bool AuxiliaryIndexCompactTask::executeStep()
{
    switch (state)
    {
        ...
        case State::NEED_EXECUTE:
        {
            try
            {
                if (build_auxiliary_index_part_task && build_auxiliary_index_part_task->execute())
                    return true;
            ...
            catch (...)
            {
                ...
                if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
                    storage_ref.postponeForResourceFailure("disk write failed for AuxiliaryIndex compact task");
                writeTaskLog(
                    AuxiliaryIndexLogElement::Type::ERROR,
                    "execute",
                    /*duration_ms=*/0,
        ...
        case State::NEED_FINISH:
        {
            ...
            catch (...)
            {
                if (entry && entry->future_part)
                    storage_ref.recordTaskFailure(*entry->future_part, getCurrentExceptionMessage(false));
                if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
                    storage_ref.postponeForResourceFailure("disk commit failed for AuxiliaryIndex compact task");
                writeTaskLog(
                    AuxiliaryIndexLogElement::Type::ERROR,
                    "finish",
                    /*duration_ms=*/0,
        ...
        case State::SUCCESS:
            throw Exception(ErrorCodes::LOGICAL_ERROR, "AuxiliaryIndex compact task in SUCCESS state must not be executed again");
    }
    UNREACHABLE();
}

void AuxiliaryIndexCompactTask::prepare()
{
    writeTaskLog(
        AuxiliaryIndexLogElement::Type::REFRESH_START,
        "prepare",
        /*duration_ms=*/0,
        /*error_code=*/0,
        /*error_message=*/{});
    ...
    {
        if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
            storage_ref.postponeForResourceFailure("disk reservation failed for AuxiliaryIndex compact task");
        writeTaskLog(
            AuxiliaryIndexLogElement::Type::ERROR,
            "prepare",
            /*duration_ms=*/0,
    ...
    build_auxiliary_index_part_task = std::make_unique<BuildTask>(
        source_snapshot,
...
void AuxiliaryIndexCompactTask::finish()
{
    Stopwatch watch;

    new_auxiliary_index_part = build_auxiliary_index_part_task->getFuture().get();
    if (!new_auxiliary_index_part)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "AuxiliaryIndex compact task did not produce a part");

    new_auxiliary_index_part->uuid = entry->future_part->new_part_uuid;
    new_auxiliary_index_part->version.setCreationTID(Tx::PrehistoricTID, nullptr);

    auto entries = StorageANN::parseCoverageJsonFromMiPart(*new_auxiliary_index_part);
    String skip_reason;
    if (!storage_ref.shouldCommitBuildOrCompactOutput(entries, "Compact", skip_reason))
    {
        writeTaskLog(
            AuxiliaryIndexLogElement::Type::REFRESH_FINISH,
            "skip_stale",
            watch.elapsedMilliseconds(),
            /*error_code=*/0,
            skip_reason,
            new_auxiliary_index_part->rows_count,
            new_auxiliary_index_part->getBytesOnDisk());
    ...
    auto replaced_parts = AuxiliaryIndexPartCommitter::commitReplacingPart(
        storage_ref,
        inner_storage_holder,
        new_auxiliary_index_part,
        *entry->future_part);
    cleanup_on_commit_failure.release();
    assertReplacedPartsMatchInput(replaced_parts, input_auxiliary_index_parts, new_auxiliary_index_part->name);

    std::vector<UUID> retired_uuids;
    retired_uuids.reserve(input_auxiliary_index_parts.size());
    for (const auto & part : input_auxiliary_index_parts)
    {
        if (!part)
            continue;
    ...
    storage_ref.recordCompactCommit(new_auxiliary_index_part->uuid, retired_uuids, entries);
    if (entry && entry->future_part)
        storage_ref.clearTaskFailure(*entry->future_part);
...
void AuxiliaryIndexCompactTask::cleanupTemporaryStorages(bool remove_output_storage) noexcept
{
    if (remove_output_storage)
    {
        ...
        {
            tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to remove AuxiliaryIndex compact tmp output");
        }
    }
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to remove AuxiliaryIndex compact tmp intermediate");
    }
...
void AuxiliaryIndexCompactTask::cleanupAfterFailedCommit() noexcept
{
    ...
    const bool remove_output_storage
        = !new_auxiliary_index_part || new_auxiliary_index_part->getState() != MergeTreeDataPartState::Active;
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to finalize AuxiliaryIndex compact entry after commit failure");
    }
}

void AuxiliaryIndexCompactTask::cancel() noexcept
{
    if (build_auxiliary_index_part_task)
    {
        build_auxiliary_index_part_task->cancel();
        if (!new_auxiliary_index_part)
            new_auxiliary_index_part = build_auxiliary_index_part_task->getUnfinishedPart();
    }
    build_auxiliary_index_part_task.reset();

    if (new_auxiliary_index_part)
    {
        new_auxiliary_index_part->removeIfNeeded();
        output_storage.reset();
    }
