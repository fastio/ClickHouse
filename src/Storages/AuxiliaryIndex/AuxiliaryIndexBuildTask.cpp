...
AuxiliaryIndexBuildTask::AuxiliaryIndexBuildTask(
    StorageANN & storage_,
...
AuxiliaryIndexBuildTask::~AuxiliaryIndexBuildTask() = default;

StorageID AuxiliaryIndexBuildTask::getStorageID() const
{
    return storage_ref.getStorageID();
}

String AuxiliaryIndexBuildTask::getQueryId() const
{
...
void AuxiliaryIndexBuildTask::writeTaskLog(
...
void AuxiliaryIndexBuildTask::onCompleted()
{
    if (task_result_callback)
        task_result_callback(true);
}

bool AuxiliaryIndexBuildTask::executeStep()
{
    switch (state)
    {
        ...
        case State::NEED_EXECUTE:
        {
            ...
            {
                tryLogCurrentException(__PRETTY_FUNCTION__, "Exception in AuxiliaryIndex AuxiliaryIndexBuildTask::executeStep");
                if (entry && entry->future_part)
        ...
        case State::NEED_FINISH:
        {
            ...
            {
                tryLogCurrentException(__PRETTY_FUNCTION__, "Exception finishing AuxiliaryIndex build task");
                if (entry && entry->future_part)
                    storage_ref.recordTaskFailure(*entry->future_part, getCurrentExceptionMessage(false));
                writeTaskLog(
                    AuxiliaryIndexLogElement::Type::ERROR,
        ...
        case State::SUCCESS:
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                "AuxiliaryIndex AuxiliaryIndexBuildTask in SUCCESS state must not be executed again");
    }
    UNREACHABLE();
}

void AuxiliaryIndexBuildTask::prepare()
{
    writeTaskLog(
        AuxiliaryIndexLogElement::Type::BUILD_START,
        "prepare",
        /*duration_ms=*/0,
        /*error_code=*/0,
        /*error_message=*/{});
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Exception preparing AuxiliaryIndex build storage");
        if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
            storage_ref.postponeForResourceFailure("disk reservation failed for AuxiliaryIndex build task");
        writeTaskLog(
            AuxiliaryIndexLogElement::Type::ERROR,
            "prepare",
            /*duration_ms=*/0,
...
void AuxiliaryIndexBuildTask::finish()
{
    Stopwatch watch;

    new_auxiliary_index_part = build_auxiliary_index_part_task->getFuture().get();
    if (!new_auxiliary_index_part)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "AuxiliaryIndex build task did not produce a part");
    ...
    {
        writeTaskLog(
            AuxiliaryIndexLogElement::Type::BUILD_FINISH,
            "skip_stale",
    ...
    FailPointInjection::pauseFailPoint(FailPoints::auxiliary_index_build_pause_in_finish);
    ...
    if (storage_ref.isShuttingDown())
        throw Exception(ErrorCodes::ABORTED, "AuxiliaryIndex build aborted by storage shutdown");

    AuxiliaryIndexPartCommitter::commitNewPart(
        storage_ref,
        inner_storage_holder,
    ...
    storage_ref.recordBuildCommit(new_auxiliary_index_part->uuid, entries);
    if (entry && entry->future_part)
        storage_ref.clearTaskFailure(*entry->future_part);
...
void AuxiliaryIndexBuildTask::cleanupTemporaryStorages(bool remove_output_storage) noexcept
{
    if (remove_output_storage)
    {
        ...
        {
            tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to remove AuxiliaryIndex build tmp output");
        }
    }
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to remove AuxiliaryIndex build tmp intermediate");
    }
...
void AuxiliaryIndexBuildTask::cleanupAfterFailedCommit() noexcept
{
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to finalize AuxiliaryIndex build entry after commit failure");
    }
}

void AuxiliaryIndexBuildTask::cancel() noexcept
{
