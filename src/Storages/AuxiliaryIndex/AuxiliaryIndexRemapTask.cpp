...
AuxiliaryIndexRemapTask::AuxiliaryIndexRemapTask(
    StorageANN & storage_,
    ...
    , affected_auxiliary_index_parts(std::move(affected_auxiliary_index_parts_))
...
{
    if (remap_kind == AuxiliaryIndexRemapKind::ObsoleteCoverageCleanup)
    {
    ...
    else if (remap_kind == AuxiliaryIndexRemapKind::MergeLineage
        || remap_kind == AuxiliaryIndexRemapKind::MutationLineage)
    {
    ...
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "AuxiliaryIndex remap task requires an explicit remap kind");
    }

    for (const auto & part : affected_auxiliary_index_parts)
        priority.value += part->getBytesOnDisk();
}

AuxiliaryIndexRemapTask::~AuxiliaryIndexRemapTask() = default;

StorageID AuxiliaryIndexRemapTask::getStorageID() const
{
    return storage_ref.getStorageID();
}

String AuxiliaryIndexRemapTask::getQueryId() const
{
...
void AuxiliaryIndexRemapTask::writeTaskLog(
    ...
    UInt64 bytes_added) const
{
    ...
    element.input_auxiliary_index_parts = collectPartNames(affected_auxiliary_index_parts);
...
void AuxiliaryIndexRemapTask::onCompleted()
{
    if (task_result_callback)
        task_result_callback(true);
}

bool AuxiliaryIndexRemapTask::executeStep()
{
    switch (state)
    {
        ...
        case State::NEED_EXECUTE:
        {
            try
            {
                if (remap_auxiliary_index_part_task && remap_auxiliary_index_part_task->execute())
                    return true;
            ...
            {
                tryLogCurrentException(__PRETTY_FUNCTION__, "Exception in AuxiliaryIndex AuxiliaryIndexRemapTask::executeStep");
                if (entry && entry->future_part)
                ...
                if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
                    storage_ref.postponeForResourceFailure("disk write failed for AuxiliaryIndex remap task");
                writeTaskLog(
        ...
        case State::NEED_FINISH:
        {
            ...
            {
                tryLogCurrentException(__PRETTY_FUNCTION__, "Exception finishing AuxiliaryIndex remap task");
                if (entry && entry->future_part)
                    storage_ref.recordTaskFailure(*entry->future_part, getCurrentExceptionMessage(false));
                if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
                    storage_ref.postponeForResourceFailure("disk commit failed for AuxiliaryIndex remap task");
                writeTaskLog(
                    AuxiliaryIndexLogElement::Type::ERROR,
                    "finish",
                    /*duration_ms=*/0,
        ...
        case State::SUCCESS:
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                "AuxiliaryIndex AuxiliaryIndexRemapTask in SUCCESS state must not be executed again");
    }
    UNREACHABLE();
}

void AuxiliaryIndexRemapTask::prepare()
{
    writeTaskLog(
    ...
    try
    {
        ...
        reserved_spaces.reserve(affected_auxiliary_index_parts.size());
        for (const auto & part : affected_auxiliary_index_parts)
        {
            if (!part)
                continue;
        ...
        remap_auxiliary_index_part_task = std::make_unique<RemapTask>(
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Exception preparing AuxiliaryIndex remap task");
        if (getCurrentExceptionCode() == ErrorCodes::NOT_ENOUGH_SPACE)
            storage_ref.postponeForResourceFailure("disk reservation failed for AuxiliaryIndex remap task");
        reserved_spaces.clear();
        writeTaskLog(
            AuxiliaryIndexLogElement::Type::ERROR,
            "prepare",
...
void AuxiliaryIndexRemapTask::finish()
{
    Stopwatch watch;

    new_auxiliary_index_parts = remap_auxiliary_index_part_task->getFuture().get();
    for (auto & part : new_auxiliary_index_parts)
    {
        if (part)
    ...
    if (entry && entry->future_part && !storage_ref.shouldCommitRemapOutput(*entry->future_part, skip_reason))
    {
        ...
        for (const auto & part : new_auxiliary_index_parts)
        {
            if (!part)
                continue;
    ...
    const size_t pair_count = std::min(new_auxiliary_index_parts.size(), affected_auxiliary_index_parts.size());
    std::vector<CoverageMap::RemapCommit> commits;
    commits.reserve(pair_count);
    for (size_t i = 0; i < pair_count; ++i)
    {
        if (!new_auxiliary_index_parts[i] || !affected_auxiliary_index_parts[i])
            continue;
        auto incoming = StorageANN::parseCoverageJsonFromMiPart(*new_auxiliary_index_parts[i]);
        commits.push_back({
            new_auxiliary_index_parts[i]->uuid,
            affected_auxiliary_index_parts[i]->uuid,
            std::move(incoming),
        });
    }
    ...
    try
    {
        AuxiliaryIndexPartCommitter::commitNewParts(
            storage_ref,
            inner_storage_holder,
            new_auxiliary_index_parts,
            *entry->future_part);
    }
    catch (...)
    {
    ...
    /// see the matching comment in `AuxiliaryIndexBuildTask::finish`. Each new materialized-index-part
    /// retires exactly one old materialized-index-part (1:1 mapping by index in AuxiliaryIndexRemapContext);
...
void AuxiliaryIndexRemapTask::cleanupAfterFailedCommit() noexcept
{
    remap_auxiliary_index_part_task.reset();
    ...
    for (auto & part : new_auxiliary_index_parts)
    {
        ...
        {
            tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to remove AuxiliaryIndex remap tmp output");
        }
    }

    new_auxiliary_index_parts.clear();
    reserved_spaces.clear();
    ...
    {
        tryLogCurrentException(__PRETTY_FUNCTION__, "Failed to finalize AuxiliaryIndex remap entry after commit failure");
    }
}

void AuxiliaryIndexRemapTask::cancel() noexcept
{
    if (remap_auxiliary_index_part_task)
    {
        remap_auxiliary_index_part_task->cancel();
        if (new_auxiliary_index_parts.empty())
            new_auxiliary_index_parts = remap_auxiliary_index_part_task->getUnfinishedParts();
    }
    remap_auxiliary_index_part_task.reset();
