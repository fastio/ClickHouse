...
StorageReplicatedMergeTree::AuxiliaryIndexKeeperChecks getKeeperChecks(
    StorageReplicatedMergeTree & replicated,
    const FutureAuxiliaryIndexPart & future_part)
{
    StorageReplicatedMergeTree::AuxiliaryIndexKeeperChecks keeper_checks;
    ...
    const FutureAuxiliaryIndexPart & future_part)
{
    ...
    auto component_guard = Coordination::setCurrentComponent("AuxiliaryIndexPartCommitter::commitNewPartToReplicatedInner");
    ...
    if (!replaced_parts.empty())
        throw Exception(
            ...
            "AuxiliaryIndex Build part {} unexpectedly replaced {} existing replicated inner parts",
            expected_part_name,
            replaced_parts.size());
}
...
void AuxiliaryIndexPartCommitter::commitNewPart(
    StorageANN & storage,
...
void AuxiliaryIndexPartCommitter::commitNewParts(
    StorageANN & storage,
    const StoragePtr & inner_table_snapshot,
...
MergeTreeData::DataPartsVector AuxiliaryIndexPartCommitter::commitReplacingPart(
    StorageANN & storage,
    const StoragePtr & inner_table_snapshot,
    MergeTreeData::MutableDataPartPtr & part,
    const FutureAuxiliaryIndexPart & future_part)
