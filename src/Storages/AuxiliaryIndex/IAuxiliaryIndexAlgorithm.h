...
struct ReadyAuxiliaryIndexPart
{
    DataPartStoragePtr storage;
    std::vector<CoveredSourcePart> covered_source_parts;
};

struct ReadyAuxiliaryIndexPartSnapshot
{
    std::vector<ReadyAuxiliaryIndexPart> parts;
};
...
{
    DataPartStoragePtr auxiliary_index_part_storage;
...
{
    std::vector<InternalHitSet> per_auxiliary_index_part;
};
...
struct CoverageSnapshot
{
    ...
    size_t ready_auxiliary_index_parts = 0;
...
/// Pluggable algorithm surface for AuxiliaryIndex tables.
///
...
/// land.
class IAuxiliaryIndexAlgorithm
{
public:
    virtual ~IAuxiliaryIndexAlgorithm() = default;
    ...
    virtual void initialize(const AuxiliaryIndexContext & ctx) = 0;
    ...
    virtual std::unique_ptr<IAuxiliaryIndexAlgorithm> cloneForBuild() const = 0;
    ...
    virtual UInt64 estimateMergeBytes(UInt64 input_auxiliary_index_bytes, UInt64 /*input_source_rows*/) const
    {
        return input_auxiliary_index_bytes;
    }
...
using AuxiliaryIndexAlgorithmPtr = std::unique_ptr<IAuxiliaryIndexAlgorithm>;
