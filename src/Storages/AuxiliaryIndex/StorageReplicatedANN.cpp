...
StorageReplicatedANN::StorageReplicatedANN(
...
bool StorageReplicatedANN::scheduleDataProcessingJob(BackgroundJobsAssignee & assignee)
{
    auto inner_table_snapshot = getInnerTable();
    auto * replicated = dynamic_cast<StorageReplicatedMergeTree *>(inner_table_snapshot.get());
    if (!replicated)
        return StorageANN::scheduleDataProcessingJob(assignee);
    ...
    if (!replicated->tryAcquireAuxiliaryIndexLeaderLease(lease_payload, lease_path))
    {
    ...
    try
    {
        const bool scheduled = StorageANN::scheduleDataProcessingJob(assignee);
        if (!scheduled)
