...
/** Mid-layer background Build task for a AuxiliaryIndex table.
  *
  ...
  *   - Constructs a Temporary-state `MergeTreeDataPartAuxiliaryIndex` at
  ...
  * `StorageANN` can follow the existing `MergePlainMergeTreeTask`
  * blueprint with minimal friction.
  */
class BuildTask
{
public:
    static constexpr auto TEMP_DIRECTORY_PREFIX = "tmp_auxiliary_index_build_";
...
private:
    ...
    struct GlobalRuntimeContext : public IStageRuntimeContext
    {
        ...
        IAuxiliaryIndexAlgorithm * algorithm{nullptr};
    ...
    /// then construct the Temporary-state MergeTreeDataPartAuxiliaryIndex.
