...
ANNAlgorithmFactory & ANNAlgorithmFactory::instance()
{
    ...
    static const bool initialized = []
    {
        ...
        factory.registerFamily(
            "ann",
            [](const String & impl, const ASTPtr & build_params, const AuxiliaryIndexContext & /*ctx*/) -> AuxiliaryIndexAlgorithmPtr
            {
#if USE_DISKANN
                if (impl == "diskann")
                {
...
void ANNAlgorithmFactory::registerFamily(const String & family_name, AlgorithmCreator creator, Strings supported_impls)
{
    ...
    if (!inserted)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "AuxiliaryIndex family '{}' is already registered", family_name);
}

AuxiliaryIndexAlgorithmPtr ANNAlgorithmFactory::get(
    const String & family,
    const String & impl,
    ...
    const AuxiliaryIndexContext & ctx) const
{
    ...
    if (it == families.end())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown AuxiliaryIndex family: '{}'", family);
