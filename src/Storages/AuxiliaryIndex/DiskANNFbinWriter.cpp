...
DiskANNFbinWriter::~DiskANNFbinWriter()
{
    if (!finalized)
    {
        ...
        /// (downstream cleanup of the temporary build storage will remove the partial file).
        try
        {
...
void DiskANNFbinWriter::finalize()
{
    ...
    if (tail_pos < 0)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "DiskANNFbinWriter: failed to determine intermediate file size ({})", tail_pos);
    /// Note: only the two UInt32 header fields (npoints, vector_dim) are 4 bytes.
    /// The file body itself can be arbitrarily large; off_t (signed 64-bit) addresses
    /// up to 8 EB. A previous check rejected files > 4 GiB, blocking high-dim 1M-row
    /// indexes (1M × 1536 × 4 B = 5.7 GiB) and any 1B-class build. Removed.
