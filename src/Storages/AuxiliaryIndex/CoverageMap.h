...
/// Process-wide map kept on `StorageANN` that materialises which
/// source parts the index already covers. The reconciler reads it (via
/// `coveredSourceUuids`); AuxiliaryIndexBuildTask appends to it after a successful commit;
/// AuxiliaryIndexRemapTask atomically swaps an old materialized-index-part for a new one; SYSTEM SYNC waits
...
public:
    ...
    /// Replace the whole map. Used by `StorageANN::startup` to
    ...
    /// Add one materialized-index-part's manifest to the map. Called from AuxiliaryIndexBuildTask::finish
    /// after the transaction has committed (and the lock has been released).
    void appendFromBuild(UUID auxiliary_index_part_uuid, std::vector<CoverageEntry> entries);
    ...
    /// not an error.
    void dropMiPart(UUID auxiliary_index_part_uuid);

    /// Forget everything. Used by `StorageANN::drop`.
    void clear();
...
private:
    ...
    /// auxiliary_index_part_uuid -> entries it contributes. Authoritative.
    std::unordered_map<UUID, std::vector<CoverageEntry>> auxiliary_index_to_entries;
