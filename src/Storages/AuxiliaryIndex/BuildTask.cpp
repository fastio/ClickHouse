...
{
    extern const MergeTreeSettingsUInt64 auxiliary_index_segment_size_rows;
}
...
constexpr UInt32 MAX_PART_UUID_ID = AuxiliaryIndexPartReverseLookup::TOMBSTONE_PART_UUID_ID - 1;
...
struct BuildTask::ReadColumnsWriteLocatorAndPrepareStage : public IStage
{
    ...
    void processBlock(const Block & block)
    {
        ...
        for (size_t i = 0; i < num_rows; ++i)
        {
            ...
            AuxiliaryIndexPartReverseLookup::writeLocatorEntry(
        ...
        if (inserted)
        {
            ...
            if (id > MAX_PART_UUID_ID)
                throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "AuxiliaryIndex part_uuid_table overflow: more than {} distinct UUIDs", MAX_PART_UUID_ID);
    ...
    UInt64 getSegmentSizeRows() const
    {
        ...
        if (global_ctx->storage)
            return (*global_ctx->storage->getSettings())[MergeTreeSetting::auxiliary_index_segment_size_rows];
...
/// MergeTreeDataPartAuxiliaryIndex. The full implementation lands in a
...
struct BuildTask::FinalizeMetadataStage : public IStage
{
    ...
    bool execute() override
    {
        ...
        /// and row count. Read back by `StorageANN::startup` to
        /// rebuild `CoverageMap` after process restart, and by `AuxiliaryIndexRemapTask` as
    ...
    void writeStandardMetadata() const
    {
        ...
        if (global_ctx->new_auxiliary_index_part)
        {
            auto & part = global_ctx->new_auxiliary_index_part;
    ...
    void constructNewMiPart() const
    {
        ...
        part_info = markAsAuxiliaryIndexPartInfo(std::move(part_info));
        ...
        global_ctx->new_auxiliary_index_part = std::move(new_part);
    }
...
BuildTask::BuildTask(
    ...
    IAuxiliaryIndexAlgorithm * algorithm_,
    StorageANN * storage_,
    ...
    , stages(makeStages())
{
    ...
    {
        const auto source_partition_range = getAuxiliaryIndexSourcePartitionRange(global_ctx->source_parts);
        ...
        if (part_info.getPartitionId() != getAuxiliaryIndexPhysicalPartitionId(global_ctx->source_partition_id)
...
bool BuildTask::execute()
{
    ...
    try
    {
        ...
        if (stages_iterator == stages.end())
        {
            ...
            {
                global_ctx->promise.set_value(global_ctx->new_auxiliary_index_part);
...
{
    return global_ctx->new_auxiliary_index_part;
}
