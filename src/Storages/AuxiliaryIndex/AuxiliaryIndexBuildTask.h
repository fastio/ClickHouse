...
class AuxiliaryIndexBuildTask : public IExecutableTask
{
public:
    AuxiliaryIndexBuildTask(
        StorageANN & storage_,
    ...
    ~AuxiliaryIndexBuildTask() override;
...
private:
    ...
    /// `source_snapshot`, `new_auxiliary_index_part` and the inner BuildTask
    ...
    /// later build). Declared before `build_auxiliary_index_part_task`
    /// so the inner task — which holds a bare pointer to this instance —
    /// is destroyed first.
    AuxiliaryIndexAlgorithmPtr build_algorithm;
...
using BuildTaskPtr = std::shared_ptr<AuxiliaryIndexBuildTask>;
