...
class AuxiliaryIndexSchedulerState
{
public:
    ...
    {
        UUID source_uuid;
        std::vector<UUID> ready_covering_auxiliary_index_parts;
        std::optional<UUID> primary_covering_auxiliary_index_part;
    ...
    struct TaskState
    {
        ...
        UUID output_auxiliary_index_part_uuid;
    ...
    void clear()
    {
        ...
        reserved_auxiliary_index_part_uuids.clear();
        ready_auxiliary_index_part_to_source_uuids.clear();
    ...
    void replaceReadyCoverage(const std::vector<std::pair<UUID, std::vector<CoverageEntry>>> & entries)
    {
        ...
        for (const auto & [auxiliary_index_part_uuid, covered_sources] : entries)
            appendReadyCoverage(auxiliary_index_part_uuid, covered_sources);
    }

    void appendReadyCoverage(UUID auxiliary_index_part_uuid, const std::vector<CoverageEntry> & entries)
    {
        auto & source_uuids = ready_auxiliary_index_part_to_source_uuids[auxiliary_index_part_uuid];
        for (const auto & entry : entries)
        {
            ...
            if (std::find(state.ready_covering_auxiliary_index_parts.begin(), state.ready_covering_auxiliary_index_parts.end(), auxiliary_index_part_uuid)
                == state.ready_covering_auxiliary_index_parts.end())
                state.ready_covering_auxiliary_index_parts.push_back(auxiliary_index_part_uuid);
            if (!state.primary_covering_auxiliary_index_part)
                state.primary_covering_auxiliary_index_part = auxiliary_index_part_uuid;
        }
    }
    ...
    {
        dropReadyMiPart(retired_auxiliary_index_part_uuid);
        appendReadyCoverage(new_auxiliary_index_part_uuid, incoming);
    }

    void applyRemapBatch(const std::vector<CoverageMap::RemapCommit> & commits)
    {
        for (const auto & commit : commits)
            dropReadyMiPart(commit.retired_auxiliary_index_part_uuid);
        for (const auto & commit : commits)
            appendReadyCoverage(commit.new_auxiliary_index_part_uuid, commit.incoming);
    }
        ...
        UUID output_auxiliary_index_part_uuid)
    {
        ...
        task.output_auxiliary_index_part_uuid = output_auxiliary_index_part_uuid;
    ...
    bool reserveRemapLineage(
        const String & task_id,
        const std::vector<UUID> & auxiliary_index_part_uuids,
        const std::vector<UUID> & source_uuids,
        UUID output_auxiliary_index_part_uuid)
    {
        if (task_id.empty() || auxiliary_index_part_uuids.empty() || tasks.contains(task_id))
            return false;

        for (const auto & uuid : auxiliary_index_part_uuids)
        {
            if (reserved_auxiliary_index_part_uuids.contains(uuid))
                return false;
        }
        for (const auto & uuid : source_uuids)
        {
        ...
        task.input_source_uuids = source_uuids;
        task.input_auxiliary_index_part_uuids = auxiliary_index_part_uuids;
        task.output_auxiliary_index_part_uuid = output_auxiliary_index_part_uuid;

        tasks.emplace(task_id, std::move(task));
        for (const auto & uuid : auxiliary_index_part_uuids)
            reserved_auxiliary_index_part_uuids.emplace(uuid, task_id);
        for (const auto & uuid : source_uuids)
        {
            reserved_source_uuids.emplace(uuid, task_id);
            coverage[uuid].source_uuid = uuid;
    ...
    void releaseTask(const String & task_id)
    {
        ...
        for (const auto & uuid : task.input_source_uuids)
        {
            ...
            if (coverage_it != coverage.end())
            {
                ...
                if (coverage_it->second.ready_covering_auxiliary_index_parts.empty() && coverage_it->second.pending_tasks.empty())
                    coverage.erase(coverage_it);
            }
        }

        for (const auto & uuid : task.input_auxiliary_index_part_uuids)
        {
            auto reserved_it = reserved_auxiliary_index_part_uuids.find(uuid);
            if (reserved_it != reserved_auxiliary_index_part_uuids.end() && reserved_it->second == task_id)
                reserved_auxiliary_index_part_uuids.erase(reserved_it);
        }
    ...
    {
        return ready_auxiliary_index_part_to_source_uuids.size();
    }
    ...
    {
        return reserved_auxiliary_index_part_uuids.contains(uuid);
    }

    bool isMiPartReservedBy(UUID uuid, const String & task_id) const
    {
        auto it = reserved_auxiliary_index_part_uuids.find(uuid);
        return it != reserved_auxiliary_index_part_uuids.end() && it->second == task_id;
    }
    ...
    UInt64 obsoleteReadySourceCount() const
    {
        ...
        for (const auto & [uuid, state] : coverage)
        {
            ...
            if (source_it != sources.end()
                ...
                && !state.ready_covering_auxiliary_index_parts.empty())
                ++count;
        }
        return count;
    }
    ...
    ObservabilitySnapshot getObservabilitySnapshot() const
    {
        ...
        snapshot.ready_auxiliary_index_part_count = ready_auxiliary_index_part_to_source_uuids.size();
...
private:
    ...
    void dropReadyMiPart(UUID auxiliary_index_part_uuid)
    {
        auto source_it = ready_auxiliary_index_part_to_source_uuids.find(auxiliary_index_part_uuid);
        if (source_it == ready_auxiliary_index_part_to_source_uuids.end())
            return;
        ...
        for (const auto & source_uuid : source_it->second)
        {
            ...
            auto & ready = coverage_it->second.ready_covering_auxiliary_index_parts;
            ready.erase(std::remove(ready.begin(), ready.end(), auxiliary_index_part_uuid), ready.end());
            if (coverage_it->second.primary_covering_auxiliary_index_part == auxiliary_index_part_uuid)
            {
                if (ready.empty())
                    coverage_it->second.primary_covering_auxiliary_index_part.reset();
                else
                    coverage_it->second.primary_covering_auxiliary_index_part = ready.front();
            }
        ...
        ready_auxiliary_index_part_to_source_uuids.erase(source_it);
    }
    ...
    std::unordered_map<UUID, String> reserved_auxiliary_index_part_uuids;
    std::unordered_map<UUID, std::vector<UUID>> ready_auxiliary_index_part_to_source_uuids;
