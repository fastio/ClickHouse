...
#include <Storages/AuxiliaryIndex/DiskANNFbinWriter.h>
...
{
    extern const Event AuxiliaryIndexDiskANNBuildStarted;
    extern const Event AuxiliaryIndexDiskANNBuildFinished;
    extern const Event AuxiliaryIndexDiskANNBuildFailed;
    extern const Event AuxiliaryIndexDiskANNSearchStarted;
    extern const Event AuxiliaryIndexDiskANNSearchFinished;
    extern const Event AuxiliaryIndexDiskANNSearchFailed;
}
...
std::unique_ptr<IAuxiliaryIndexAlgorithm> DiskANNAlgorithm::cloneForBuild() const
{
...
void DiskANNAlgorithm::validateIndexedExpression(const ASTPtr & indexed_expression, const StorageInMemoryMetadata & source_metadata)
{
    ...
    /// validateAuxiliaryIndexPrerequisites); unwrap it transparently.
...
void DiskANNAlgorithm::initialize(const AuxiliaryIndexContext & /*ctx*/)
{
    initialized = true;
}
...
{
    ProfileEvents::increment(ProfileEvents::AuxiliaryIndexDiskANNSearchStarted);
    bool search_finished = false;
    scope_guard failed_search_guard = [&search_finished]
    {
        if (!search_finished)
            ProfileEvents::increment(ProfileEvents::AuxiliaryIndexDiskANNSearchFailed);
    };
    ...
    result.per_auxiliary_index_part.reserve(ready_parts.parts.size());
    ...
    for (const auto & ready_part : ready_parts.parts)
    {
        ...
        hit_set.auxiliary_index_part_storage = part_storage;
        hit_set.internal_ids = std::move(hits);
        hit_set.distances = std::move(distances);
        result.per_auxiliary_index_part.push_back(std::move(hit_set));
    }

    ProfileEvents::increment(ProfileEvents::AuxiliaryIndexDiskANNSearchFinished);
...
void DiskANNAlgorithm::prepareBuild(const AlgorithmBuildContext & ctx, const Block & indexed_columns_batch)
{
    ...
    if (!fbin_writer)
    {
        ProfileEvents::increment(ProfileEvents::AuxiliaryIndexDiskANNBuildStarted);
...
void DiskANNAlgorithm::buildAlgorithmPrivate(const AlgorithmBuildContext & ctx)
{
    ...
    {
        ProfileEvents::increment(ProfileEvents::AuxiliaryIndexDiskANNBuildFailed);
        throw;
    }

    ProfileEvents::increment(ProfileEvents::AuxiliaryIndexDiskANNBuildFinished);
}
