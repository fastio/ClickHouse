...
BatchSource::BatchSource(
    ...
    , coordinator(std::move(coordinator_))
    , discard_output_columns(discard_output_columns_)
...
Chunk BatchSource::generate()
{
    ...
    Chunk chunk;
    if (discard_output_columns)
    {
        chunk = Chunk(Columns{}, batch_rows);
    }
    else
    {
        ...
        chunk = converter->arrowTableToCHChunk(*table, (*table)->num_rows(), /* metadata */ nullptr, /* block_missing_values */ nullptr);
...
Chunk CountSource::generate()
{
    ...
    try
    {
        const auto rows = scan.predicate || !scan.fragment_ids.empty()
            ? dataset.countRows(scan.snapshot, scan.predicate, scan.fragment_ids, cancellation->handle())
            : dataset.totalRows(scan.snapshot, cancellation->handle());
        if (!rows)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "`Lance` count source did not receive a row count");

        ProfileEvents::increment(ProfileEvents::LanceRowsRead, *rows);
        addTotalRowsApprox(*rows);
        Columns physical_columns;
        physical_columns.reserve(output_header.columns() - requested_virtual_columns.size());
        for (const auto & column : output_header)
        {
            if (requested_virtual_columns.contains(column.name))
                continue;
            auto result_column = column.type->createColumn();
            result_column->insertManyDefaults(*rows);
            physical_columns.emplace_back(std::move(result_column));
        }
        Chunk chunk(std::move(physical_columns), *rows);
        addVirtualColumns(chunk, requested_virtual_columns, virtual_values, context, format_settings);
        return chunk;
    }
