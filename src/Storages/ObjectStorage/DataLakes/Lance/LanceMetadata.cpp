    ...
    Lance::ReadVirtualValues virtual_values) const
{
    ...
    bool use_output_physical_columns = false;
    if (!effective_need_only_count && scan_projection.empty())
    {
        ...
        {
            /// The planner may add a physical column to the Source header solely to
            /// establish row counts while leaving `requested_columns` empty.
            use_output_physical_columns = true;
        }
        else
        {
            if (!pinned_physical_columns)
                pinned_physical_columns = dataset.tableSchema(snapshot, local_context, cancel_handle);
            if (pinned_physical_columns->empty())
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot scan a `Lance` dataset without physical columns");
            scan_projection.push_back(pinned_physical_columns->front().name);
            discard_output_columns = true;
        }
    }

    std::optional<UInt64> scan_limit;
    if (limit && *limit > 0 && !effective_need_only_count && predicate_pushdown.is_complete && fragment_ids.empty())
    ...
    if (!scan_in_order && !effective_need_only_count)
    {
    ...
    {
        return Pipe(std::make_shared<Lance::CountSource>(
            output_header,
            std::move(dataset),
