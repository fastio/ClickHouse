...
StorageSystemAuxiliaryIndexParts::StorageSystemAuxiliaryIndexParts(const StorageID & storage_id_, ColumnsDescription columns_description_)
    : IStorageSystemOneBlock(storage_id_, std::move(columns_description_))
{
}

ColumnsDescription StorageSystemAuxiliaryIndexParts::getColumnsDescription()
{
    return ColumnsDescription
    {
        {"database", std::make_shared<DataTypeString>(), "Database of the auxiliary index that owns this part."},
        {"index_name", std::make_shared<DataTypeString>(), "Name of the auxiliary index that owns this part."},
        ...
        {"tombstone_rows", std::make_shared<DataTypeUInt64>(), "Number of tombstone locator rows recorded in this AuxiliaryIndex part."},
        {"tombstone_ratio", std::make_shared<DataTypeFloat64>(), "Ratio of tombstone locator rows to rows in this AuxiliaryIndex part."},
...
void StorageSystemAuxiliaryIndexParts::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const
{
    ...
    for (const auto & [database_name, database] : databases)
    {
        ...
        for (auto tables_it = database->getTablesIterator(context); tables_it->isValid(); tables_it->next())
        {
            ...
            try
            {
                active_parts = auxiliary_index->getAccessPathPartsVectorForInternalUsage();
            }
            catch (...)
            {
                /// Mirrors system.auxiliary_indexes: a storage that rejects
                /// the snapshot mid-shutdown should not fail the whole query.
                tryLogCurrentException(getLogger("StorageSystemAuxiliaryIndexParts"));
                continue;
            }
