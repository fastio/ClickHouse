...
StorageSystemMaterializedIndexParts::StorageSystemMaterializedIndexParts(const StorageID & storage_id_, ColumnsDescription columns_description_)
    : IStorageSystemOneBlock(storage_id_, std::move(columns_description_))
{
}

ColumnsDescription StorageSystemMaterializedIndexParts::getColumnsDescription()
{
    return ColumnsDescription
    {
        {"database", std::make_shared<DataTypeString>(), "Database of the materialized index that owns this part."},
        {"index_name", std::make_shared<DataTypeString>(), "Name of the materialized index that owns this part."},
...
void StorageSystemMaterializedIndexParts::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const
{
    ...
    for (const auto & [database_name, database] : databases)
    {
        ...
        for (auto tables_it = database->getTablesIterator(context); tables_it->isValid(); tables_it->next())
        {
            ...
            try
            {
                active_parts = materialized_index->getAccessPathPartsVectorForInternalUsage();
            }
            catch (...)
            {
                /// Mirrors system.materialized_indexes: a storage that rejects
                /// the snapshot mid-shutdown should not fail the whole query.
                tryLogCurrentException(getLogger("StorageSystemMaterializedIndexParts"));
                continue;
            }
