---
description: 'System table containing one row per active materialized-index part.'
keywords: ['system table', 'materialized_index_parts']
sidebar_label: 'materialized_index_parts'
sidebar_position: 74
slug: /operations/system-tables/materialized_index_parts
title: 'system.materialized_index_parts'
doc_type: 'reference'
---

# system.materialized_index_parts {#system-materialized-index-parts}

## Description {#description}

Lists every active materialized-index-part for each `MATERIALIZED INDEX` visible to the current user. One row corresponds to one on-disk part in the inner `MergeTree` storage backing the index.
...
See [`system.materialized_indexes`](/operations/system-tables/materialized_indexes) for index-level aggregates and [`MaterializedIndex`](/engines/table-engines/mergetree-family/materialized-index) for the engine overview.

## Columns {#columns}

- `database` ([String](/sql-reference/data-types/string)) — Database of the materialized index that owns this part.
- `index_name` ([String](/sql-reference/data-types/string)) — Name of the materialized index that owns this part.
- `part_name` ([String](/sql-reference/data-types/string)) — On-disk name of the materialized-index-part.
- `part_uuid` ([UUID](/sql-reference/data-types/uuid)) — UUID of the materialized-index-part.
...
- `bytes_on_disk` ([UInt64](/sql-reference/data-types/int-uint)) — Disk footprint of the materialized-index-part in bytes.
...
After `SYSTEM SYNC MATERIALIZED INDEX`, each built part should appear with non-zero `rows` and `bytes_on_disk`:
...
- [`system.materialized_indexes`](/operations/system-tables/materialized_indexes)
