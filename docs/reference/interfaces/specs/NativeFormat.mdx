...
A physical key column of a `with_key_columns` Map can have type `Nullable(Array(Nullable(Int32)))`. For rows `[NULL, [], [NULL], [1, NULL, -2, 3]]`, its native encoding is composed in this order:

```text
01 00 00 00                    outer null map: 4 rows
0, 0, 1, 5                     array offsets: 4 little-endian UInt64 values
01 00 01 00 00                 element null map: 5 elements
0, 1, 0, -2, 3                 element values: 5 little-endian Int32 values
```

The first row is a missing key and uses an empty array placeholder. The second row is a present empty array. The third row is a present array containing one null element. Zero values shown for null elements are placeholders. The two null maps have independent lengths, determined by the row count and final array offset. No additional version or marker is introduced: this follows the standard recursive `Nullable` and `Array` encodings. The `.null` and `.null1` filenames used by `with_key_columns` part storage are not transmitted on the wire. A complete `Map(String, Array(Nullable(Int32)))` column retains the ordinary Map wire layout; it has no per-key presence bitmap.
...
The `MergeTree` setting `map_serialization_version = 'with_key_columns'` controls part storage, not the `Native` wire layout. Its restriction on introducing new map keys after the first written block when the value type contains `LowCardinality`, including inside arrays, applies to part writing and does not restrict keys in successive `Native` blocks.
