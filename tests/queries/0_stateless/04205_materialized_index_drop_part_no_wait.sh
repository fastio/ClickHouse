#!/usr/bin/env bash
# Tags: no-fasttest
# Covers MaterializedIndex part removal paths:
#   * `dropPart` / `ALTER TABLE ... DROP PART` — user-facing; runs the inner
#     partition command and refreshes coverage (see `04191_materialized_index_drop_part.sh`).
...
#     drops for active inner part names (`StorageMaterializedIndex.cpp`).
...
CREATE MATERIALIZED INDEX mi_drop_no_wait
...
SYSTEM SYNC MATERIALIZED INDEX mi_drop_no_wait;
