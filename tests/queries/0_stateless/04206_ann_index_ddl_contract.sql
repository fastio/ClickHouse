...
CREATE REFLECTION ai_ddl_idx ON ai_ddl_src (v) ENGINE = ANNIndex(diskann) SETTINGS ann_metric = 'L2', ann_dimension = 4, spann_head_ratio = 0.5; -- { serverError BAD_ARGUMENTS }
CREATE REFLECTION ai_ddl_idx ON ai_ddl_src (v) ENGINE = ANNIndex(spann) SETTINGS ann_metric = 'L2', ann_dimension = 4, diskann_max_degree = 64; -- { serverError BAD_ARGUMENTS }
CREATE REFLECTION ai_ddl_idx ON ai_ddl_src (v) ENGINE = ANNIndex(diskann) SETTINGS ann_metric = 'L2', ann_dimension = 4, ann_compact_all_replicas = 1; -- { serverError BAD_ARGUMENTS }
