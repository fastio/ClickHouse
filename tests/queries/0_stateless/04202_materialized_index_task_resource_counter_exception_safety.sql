...
-- Otherwise `materialized_index_max_global_background_tasks = 1` would block all
...
CREATE MATERIALIZED INDEX mi_res_fp
...
SETTINGS materialized_index_sync_timeout = 60,
         materialized_index_max_global_background_tasks = 1,
         materialized_index_build_min_rows = 1,
         materialized_index_build_min_parts = 1;

SYSTEM ENABLE FAILPOINT materialized_index_throw_in_try_reserve_future_part;
...
-- time out under `materialized_index_max_global_background_tasks = 1`, and
-- (2) the failpoint actually fired. Probing `enabled` after SYNC is then race-free.
SYSTEM SYNC MATERIALIZED INDEX mi_res_fp;

SELECT count() AS reserve_failpoint_fired_and_disabled
FROM system.fail_points
WHERE name = 'materialized_index_throw_in_try_reserve_future_part' AND enabled = 1;

SELECT
    materialized_index_part_count > 0 AS build_committed_after_counter_release,
