...
-- replicated `MaterializedIndex` Remap and Compact tasks. The failpoint makes
...
SELECT materialized_index_part_count >= 2 AS remap_has_two_input_parts
...
SYSTEM FLUSH LOGS materialized_index_log;

SELECT count() > 0 AS remap_finish_logged
FROM system.materialized_index_log
...
LIMIT 1
SETTINGS force_materialized_index = 'mi_repl_remap_fp';
...
CREATE MATERIALIZED INDEX mi_repl_compact_fp
ON src_repl_compact_fp (embedding)
TYPE ann('diskann', metric = 'L2', dim = 4)
ENGINE = ReplicatedMaterializedIndex('/clickhouse/tables/{database}/mi_repl_compact_fp_04197', 'r1')
SETTINGS materialized_index_sync_timeout = 60;
...
SYSTEM SYNC MATERIALIZED INDEX mi_repl_compact_fp;
...
SELECT
    materialized_index_part_count > 0 AS compact_has_part,
...
ORDER BY L2Distance(embedding, q)
LIMIT 1
SETTINGS force_materialized_index = 'mi_repl_compact_fp';
