...
CREATE MATERIALIZED INDEX mi_spann_smoke
ON src_spann_smoke (embedding)
TYPE ann('spann', metric = 'L2', dim = 16)
...
SYSTEM SYNC MATERIALIZED INDEX mi_spann_smoke;

CREATE TEMPORARY TABLE mi_spann_start AS SELECT now64(6) AS ts;

SET force_materialized_index = 'mi_spann_smoke';
...
SELECT
    log_comment,
    max(ProfileEvents['MaterializedIndexSPANNSearchStarted'] > 0) AS used_spann_search
...
CREATE MATERIALIZED INDEX mi_spann_cosine
ON src_spann_cosine (embedding)
TYPE ann('spann', metric = 'cosine', dim = 16)
ENGINE = MaterializedIndex
SETTINGS materialized_index_sync_timeout = 60,
         materialized_index_build_min_rows = 1,
         materialized_index_build_min_parts = 1;

SYSTEM SYNC MATERIALIZED INDEX mi_spann_cosine;

CREATE TEMPORARY TABLE mi_spann_cosine_start AS SELECT now64(6) AS ts;

SET force_materialized_index = 'mi_spann_cosine';
