...
SET ann_index_overfetch_factor = 1025;

DROP TABLE IF EXISTS mi_of_dis_cppdiskann SYNC;
DROP TABLE IF EXISTS src_of_dis_cppdiskann;

CREATE TABLE src_of_dis_cppdiskann (k UInt64, embedding Array(Float32))
ENGINE = MergeTree
ORDER BY k
SETTINGS assign_part_uuids = 1, enable_block_number_column = 1, enable_block_offset_column = 1;

INSERT INTO src_of_dis_cppdiskann
SELECT number, [number * 1.0, 0, 0, 0]
FROM numbers(100);

CREATE REFLECTION mi_of_dis_cppdiskann
ON src_of_dis_cppdiskann (embedding)
...
SELECT k FROM src_of_dis_cppdiskann
ORDER BY L2Distance(embedding, [41.7, 0, 0, 0])
LIMIT 10;

DROP TABLE mi_of_dis_cppdiskann SYNC;
DROP TABLE src_of_dis_cppdiskann;
