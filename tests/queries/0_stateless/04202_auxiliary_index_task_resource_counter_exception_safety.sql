...
-- Otherwise `auxiliary_index_max_global_background_tasks = 1` would block all
...
SYSTEM ENABLE FAILPOINT auxiliary_index_throw_in_try_reserve_future_part;
...
WHERE name = 'auxiliary_index_throw_in_try_reserve_future_part' AND enabled = 1;

SELECT
    auxiliary_index_part_count > 0 AS build_committed_after_counter_release,
