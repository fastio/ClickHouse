...
-- force_auxiliary_index pointing at a non-existent MI must not throw. The
-- optimizer logs a warning and falls back to cost-based selection over the
-- candidates that survived `disable_auxiliary_index`. Result remains the
-- correct top-K.
...
SET force_auxiliary_index = 'mi_does_not_exist';
