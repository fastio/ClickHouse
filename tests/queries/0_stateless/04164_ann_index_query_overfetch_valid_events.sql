...
CREATE REFLECTION mi_query_overfetch_valid
ON src_query_overfetch_valid (embedding)
ENGINE = ANNIndex(diskann)
SETTINGS ann_metric = 'L2', ann_dimension = 4,
         ann_index_sync_timeout = 20;

SYSTEM SYNC REFLECTION mi_query_overfetch_valid;
...
LIMIT 1
FORMAT Null
SETTINGS force_ann_index = 'mi_query_overfetch_valid', log_comment = 'mi_overfetch_default';

SELECT k FROM src_query_overfetch_valid
ORDER BY L2Distance(embedding, [41.7, 0, 0, 0])
LIMIT 1
FORMAT Null
SETTINGS force_ann_index = 'mi_query_overfetch_valid', log_comment = 'mi_overfetch_min', ann_index_overfetch_factor = 1;

SELECT k FROM src_query_overfetch_valid
ORDER BY L2Distance(embedding, [41.7, 0, 0, 0])
LIMIT 1
FORMAT Null
SETTINGS force_ann_index = 'mi_query_overfetch_valid', log_comment = 'mi_overfetch_max', ann_index_overfetch_factor = 1024;
