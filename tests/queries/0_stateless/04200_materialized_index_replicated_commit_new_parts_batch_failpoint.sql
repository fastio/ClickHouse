...
-- Exercises `MaterializedIndexPartCommitter::commitNewParts` on a replicated
...
CREATE MATERIALIZED INDEX mi_repl_batch_fp
ON src_repl_batch_fp (embedding)
TYPE ann('diskann', metric = 'L2', dim = 4)
ENGINE = ReplicatedMaterializedIndex('/clickhouse/tables/{database}/mi_repl_batch_fp_04200', 'r1')
...
SYSTEM SYNC MATERIALIZED INDEX mi_repl_batch_fp;
...
SELECT materialized_index_part_count >= 2 AS has_two_input_parts
...
SELECT
    materialized_index_part_count = 2 AS batch_outputs_committed,
