-- Adapted from ByConity tests/queries/4_cnch_stateless/00983_merge_map_data_loss.sql
...
CREATE TABLE test_map_01 (c_date Date,c_datetime DateTime64(3),s_map Map(String, String),i_map Map(String, Int64),f_map Map(String, Float64) ) ENGINE = MergeTree PARTITION BY c_date ORDER BY c_date TTL toDate(c_date) + toIntervalDay(30) SETTINGS index_granularity = 8192, map_serialization_version = 'with_key_columns', map_serialization_version_for_zero_level_parts = 'with_key_columns', min_rows_for_wide_part = 0, min_bytes_for_wide_part = 0;
...
OPTIMIZE TABLE test_map_01 FINAL;
