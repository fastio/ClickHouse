...
-- which is the commit path used by `ReplicatedMaterializedIndex` builds.
...
CREATE MATERIALIZED INDEX mi_repl_fp
ON src_repl_fp (embedding)
TYPE ann('diskann', metric = 'L2', dim = 4)
ENGINE = ReplicatedMaterializedIndex('/clickhouse/tables/{database}/mi_repl_fp_04189', 'r1')
...
-- and `materialized_index_part_count` would stay at 0.
SELECT
...
LIMIT 1
SETTINGS force_materialized_index = 'mi_repl_fp';
