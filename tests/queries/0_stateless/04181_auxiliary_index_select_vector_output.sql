...
-- no-parallel-replicas because the test sets `auxiliary_index_require_match = 1`
...
-- AuxiliaryIndex should still match when SELECT outputs need the vector column.

SET allow_experimental_auxiliary_index = 1;
SET enable_auxiliary_index = 1;
SET force_auxiliary_index = 'mi_select_vector_output';
SET log_queries = 1;
...
LIMIT 1
SETTINGS auxiliary_index_require_match = 1, log_comment = 'mi_select_vector_output';
...
LIMIT 1
FORMAT Null
SETTINGS auxiliary_index_require_match = 1, log_comment = 'mi_select_vector_where';
...
ORDER BY L2Distance(embedding, q)
LIMIT 1
SETTINGS auxiliary_index_require_match = 1, log_comment = 'mi_select_vector_filter_depends'; -- { serverError BAD_ARGUMENTS }
