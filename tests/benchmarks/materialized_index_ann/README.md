...
for L in 64 100 200 400 800; do
  ./docker-bench.sh --algo diskann --dataset sift1m --preset /presets/diskann_sift1m.json \
      --search materialized_index_diskann_search_list_size=$L --query-count 10000
done
jq -r '[.search_settings.materialized_index_diskann_search_list_size, .recall_at_10, .qps.c1.qps, .qps.c16.qps] | @tsv' \
...
2. **OPTIMIZE FINAL**: collapse `sift_base` to a single active part. ANN groups
   in `MaterializedIndex` are part-bound — without this step a 1B-row table
...
3. **CREATE MATERIALIZED INDEX** + **SYSTEM SYNC**: build the index over the
...
|-------------------------------------------------------------|---------|---------------------------------------------------------------|
| `materialized_index_diskann_search_list_size`               | `200`   | `L_search` candidate list size                                |
| `materialized_index_diskann_search_beam_width`              | `16`    | graph neighbours dispatched per step (I/O concurrency)        |
| `materialized_index_diskann_search_num_threads`             | `8`     | per-part searcher worker pool (open-time, in cache key)       |
| `materialized_index_diskann_search_io_limit`                | `256`   | per-part in-flight I/O cap (open-time, in cache key)          |
| `materialized_index_diskann_search_nodes_to_cache`          | `1024`  | per-part hot-node cache size (open-time, in cache key)        |
...
into the index by `CREATE MATERIALIZED INDEX`. SPTAG re-reads these from
...
|---------------------------------------------------------------|-----------------------|------------------------------------------------|
| `materialized_index_spann_search_posting_page_limit`          | `search_posting_page_limit` | SSD workspace (`m_options.m_searchPostingPageLimit`) |
| `materialized_index_spann_search_internal_result_num`         | `internal_result_num`       | SSD workspace (`m_options.m_searchInternalResultNum`) |
| `materialized_index_spann_search_max_dist_ratio`              | `max_dist_ratio`            | per-search read (`m_options.m_maxDistRatio`) |
| `materialized_index_spann_search_max_check`                   | `max_check`                 | head BKT (`m_index->SetParameter("MaxCheck", ...)`) |
| `materialized_index_spann_search_hash_table_exponent`         | `hash_table_exponent`       | head BKT (`m_index->SetParameter("HashTableExponent", ...)`) |
