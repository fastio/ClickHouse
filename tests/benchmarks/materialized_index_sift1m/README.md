# MaterializedIndex × DiskANN — SIFT-1M recall@10 benchmark

End-to-end correctness gate for the `MaterializedIndex` query fast path on
...
the cheap `04179_materialized_index_diskann_self_query_smoke.sql` smoke in
...
- A running ClickHouse server with `MaterializedIndex` available
...
```bash
cd tests/benchmarks/materialized_index_sift1m
...
`DiskIndexSearcher` per MaterializedIndex part, search throughput scales
...
| `SYNC_TIMEOUT_SEC`   | `3600`                                   | `SYSTEM SYNC MATERIALIZED INDEX` timeout (60 min)    |
...
2. `load.sh` streams the HDF5 file through
   ...
   10 k GT rows).
3. `CREATE MATERIALIZED INDEX mi_sift ... TYPE ann('diskann', ...)` with
...
4. Generates `QUERY_COUNT` per-query SELECTs of the form
   ...
   `optimizeMaterializedIndex.cpp:extractQueryParams` requires.
   `force_materialized_index = 'mi_sift'` is set so the MI fast path is the
   only path taken, and `materialized_index_require_match = 1` makes the
...
6. As a defence in depth on top of `materialized_index_require_match`, also
   asserts via `system.query_log` that every recall SELECT logged
   `MaterializedIndexDiskANNSearchStarted > 0`.
...
`src/Storages/MaterializedIndex/DiskANNAlgorithm.cpp` — exposing them as
...
settings.json                -- session settings (allow_experimental_materialized_index)
```
