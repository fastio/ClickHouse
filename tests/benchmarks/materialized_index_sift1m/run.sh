...
log "DROP IF EXISTS mi_sift + CREATE MATERIALIZED INDEX (tuned: pq_chunks=32, R=64, max=96)"
"${CLIENT[@]}" --multiquery -q "
SET allow_experimental_materialized_index = 1;
DROP TABLE IF EXISTS mi_sift SYNC;
CREATE MATERIALIZED INDEX mi_sift
ON sift_base (v)
TYPE ann('diskann',
    metric = 'L2',
    dim = 128,
...
log "SYSTEM SYNC MATERIALIZED INDEX mi_sift (timeout ${SYNC_TIMEOUT_SEC}s)"
"${CLIENT[@]}" --receive_timeout="$SYNC_TIMEOUT_SEC" \
    -q "SYSTEM SYNC MATERIALIZED INDEX mi_sift"

log "MI status:"
"${CLIENT[@]}" -q "
SELECT name, materialized_index_part_count, total_rows, total_bytes_on_disk, backlog_parts, pending_task_count
...
{
    echo "SET allow_experimental_materialized_index = 1;"
    echo "SET enable_materialized_index = 1;"
    echo "SET force_materialized_index = 'mi_sift';"
    ...
    echo "SET materialized_index_require_match = 1;"
    ...
    # which is what optimizeMaterializedIndex matches on.
    "${CLIENT[@]}" -q "
...
SELECT countIf(ProfileEvents['MaterializedIndexDiskANNSearchStarted'] > 0)
