...
Algorithm-agnostic recall + QPS harness for `ANNIndex` ANN backends
(`diskann`, `spann`). One invocation = one (algorithm, dataset, build params,
search params) point, emitted as a JSON record.

This supersedes the DiskANN-only `tests/benchmarks/ann_index_sift1m/`
...
2. **OPTIMIZE FINAL**: collapse `sift_base` to a single active part. ANN groups
   in `ANNIndex` are part-bound — without this step a 1B-row table
...
Search session settings (per-query). Pass `0` to use the built-in default for integer settings; `diskann_search_num_threads_ratio` must be greater than `0`.
...
| `diskann_search_num_threads_ratio`       | `8`     | per-part searcher worker ratio (open-time, capped at `64`)    |
...
correctness gate `04179_ann_index_diskann_self_query_smoke.sql` lives
...
* Recall correctness uses the same per-query intersect template as the
  DiskANN-only `ann_index_sift1m/run.sh` and runs under
