# ANNIndex × DiskANN — SIFT-1M recall@10 benchmark

End-to-end correctness gate for the `ANNIndex` query fast path on
...
the cheap `04179_ann_index_diskann_self_query_smoke.sql` smoke in
...
- A running ClickHouse server with `ANNIndex` available
...
```bash
cd tests/benchmarks/ann_index_sift1m
...
`DiskIndexSearcher` per ANNIndex part, search throughput scales
...
4. Generates `QUERY_COUNT` per-query SELECTs of the form
   ...
   `optimizeANNIndex.cpp:extractQueryParams` requires.
   `force_ann_index = 'mi_sift'` is set so the MI fast path is the
   only path taken, and `ann_index_require_match = 1` makes the
...
6. As a defence in depth on top of `ann_index_require_match`, also
   asserts via `system.query_log` that every recall SELECT logged
   `ANNIndexDiskANNSearchStarted > 0`.
...
`src/Storages/Reflection/ANNIndex/DiskANNAlgorithm.cpp` — exposing them as
...
settings.json                -- session settings (allow_experimental_ann_index)
```
