#!/usr/bin/env bash
# Full SIFT-1M ANNIndex recall benchmark.
#
# Steps:
...
#      force_ann_index = 'mi_sift' (so the MI fast path is taken),
...
SETTINGS ann_metric = 'L2', ann_dimension = 128, diskann_pq_chunks = 32, diskann_pruned_degree = 64, diskann_max_degree = 96, diskann_l_build = 128, diskann_alpha = 1.2, diskann_num_threads = 16,
         ann_index_sync_timeout = $SYNC_TIMEOUT_SEC;
"
...
log "MI status:"
"${CLIENT[@]}" -q "
SELECT name, ann_index_part_count, total_rows, total_bytes_on_disk, backlog_parts, pending_task_count
...
{
    echo "SET allow_experimental_ann_index = 1;"
    echo "SET enable_ann_index = 1;"
    echo "SET force_ann_index = 'mi_sift';"
    ...
    echo "SET ann_index_require_match = 1;"
    ...
    # which is what optimizeANNIndex matches on.
    "${CLIENT[@]}" -q "
...
SELECT countIf(ProfileEvents['ANNIndexDiskANNSearchStarted'] > 0)
