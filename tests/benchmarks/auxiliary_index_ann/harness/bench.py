...
def run_recall(server: ClickHouseServer, dataset: DatasetSpec, algo: AlgoSpec,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with sql_file.open("w") as f:
            f.write("SET allow_experimental_auxiliary_index = 1;\n")
            f.write("SET enable_auxiliary_index = 1;\n")
            f.write(f"SET force_auxiliary_index = '{index_name}';\n")
            f.write("SET auxiliary_index_require_match = 1;\n")
...
def run_qps(server: ClickHouseServer, dataset: DatasetSpec, algo: AlgoSpec, index_name: str,
    ...
    settings = {
        ...
        "allow_experimental_auxiliary_index": 1,
        "enable_auxiliary_index": 1,
        "force_auxiliary_index": index_name,
...
def main() -> int:
    ...
    ap.add_argument("--reuse-index", action="store_true",
                    help="skip load/optimize/CREATE INDEX — connect to an existing data dir "
                         "that already holds sift_base + the auxiliary index. Used for "
    ...
    with ClickHouseServer(binary=args.binary, data_dir=args.data_dir,
        ...
        if args.reuse_index:
            ...
            idx_count = int(srv.query(
                f"SELECT count() FROM system.auxiliary_indexes "
                f"WHERE database = currentDatabase() AND name = '{index_name}'"
            ).strip() or 0)
            if idx_count == 0:
                raise SystemExit(f"--reuse-index: no auxiliary index named {index_name!r}")
...
SELECT total_bytes_on_disk FROM system.auxiliary_indexes
