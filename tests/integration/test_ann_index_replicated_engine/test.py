...
(as opposed to a ANNIndex inside a `Replicated` database, which
is covered by `test_ann_index_replicated`).
...
The scenarios here exist only in a true multi-node cluster:
  ...
  * Leader failure mid-build: the leader's session expires (or it
    ...
    `ann_index_build_pause_in_finish` PAUSEABLE_ONCE failpoint.
...
CONFIG = ["configs/config.xml", "configs/ann_index_log.xml"]
...
def _create_mi(suffix, sync_timeout=180):
    for node in all_nodes:
        node.query(
            ...
            f"SETTINGS ann_metric = 'L2', ann_dimension = 4, ann_index_sync_timeout = {sync_timeout}, "
            f"ann_index_build_min_rows = 1, "
            f"ann_index_build_min_parts = 1"
        )


def _build_finish_count(node, suffix):
    """Count `BuildFinish` events in `system.ann_index_log` for
    the named ann index on this replica only. Each replica has
    its own log, so this is the local build count."""
    node.query("SYSTEM FLUSH LOGS ann_index_log")
    return int(
        node.query(
            f"SELECT count() FROM system.ann_index_log "
...
def test_two_replica_lease_and_fetch(started_cluster):
    ...
    try:
        ...
        # `system.ann_indexes` view.
        for node in all_nodes:
            row = node.query(
                f"SELECT ann_index_part_count, total_rows "
                f"FROM system.ann_indexes "
        ...
        for node in all_nodes:
            res = node.query(
                ...
                f"LIMIT 1 "
                f"SETTINGS force_ann_index = 'mi_{suffix}'"
            ).strip()
...
def test_leader_failover_midbuild(started_cluster):
    ...
    Sequence:
      ...
      2. Enable the `ann_index_build_pause_in_finish` PAUSEABLE_ONCE
    ...
    try:
        ...
        node1.query(
            f"SYSTEM ENABLE FAILPOINT ann_index_build_pause_in_finish"
        )
        ...
        # on the ann index return trivially with 0 rows.
        ...
        node1.query(
            f"SYSTEM WAIT FAILPOINT ann_index_build_pause_in_finish PAUSE",
            timeout=180,
        )
        ...
        for node in all_nodes:
            try:
                node.query(
                    f"SYSTEM DISABLE FAILPOINT ann_index_build_pause_in_finish"
                )
            except Exception:
                logging.exception(
...
def test_restart_recovery_round_trip(started_cluster):
    ...
    `276dc5d4425`) so that `system.ann_indexes` still reports the
