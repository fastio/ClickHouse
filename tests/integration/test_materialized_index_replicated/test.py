...
def map_names(node, db_name):
    ...
    only enumerable via `system.materialized_indexes`."""
    rows = node.query(
        f"SELECT name FROM system.materialized_indexes "
...
def test_create_drop_replication(started_cluster):
    """T-7.1 + T-7.2: a CREATE / DROP MATERIALIZED INDEX issued on one replica is
    ...
    try:
        ...
        node1.query(f"CREATE MATERIALIZED INDEX m1 ON {db}.src (vec) USING diskann()")
        node1.query(f"CREATE MATERIALIZED INDEX m2 ON {db}.src (vec) USING diskann()")

        sync_replicas(db)
        for node in all_nodes:
            assert map_names(node, db) == ["m1", "m2"], f"{node.name}: {map_names(node, db)}"

        node2.query(f"DROP MATERIALIZED INDEX {db}.m1")
        sync_replicas(db)
        for node in all_nodes:
    ...
    try:
        ...
        for node in all_nodes:
            ...
            metadata_path = node.query(
                f"SELECT metadata_path FROM system.materialized_indexes "
